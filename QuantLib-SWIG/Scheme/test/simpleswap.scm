...
(define (make-test-swap today length fixed-rate floating-spread)
  (let ((pay-fixed #t)
        (settlement-days 2)
        (fixing-days 2)
        (nominal 100)
    ...
    (deleting-let* ((fixed-day-count (new-DayCounter "30/360")
                    ...
                    (curve-day-count (new-DayCounter "act/365")
                                     delete-DayCounter)
                    (term-structure (new-FlatForward settlement 0.05
                                                     curve-day-count)
  ...
  (deleting-let ((today (Date-todays-date) delete-Date))
    (let ((fixed-rate 0.0))
      (for-each-combination ((length '(1 2 5 10 20))
        ...
        (deleting-let ((swap (make-test-swap today length fixed-rate spread)
                             delete-Instrument))
          (let ((fair-rate (SimpleSwap-fair-rate swap)))
            (deleting-let ((swap (make-test-swap today length fair-rate spread)
                                 delete-Instrument))
  ...
  (deleting-let ((today (Date-todays-date) delete-Date))
    (let ((spread 0.0))
      (for-each-combination ((length '(1 2 5 10 20))
        ...
        (deleting-let ((swap (make-test-swap today length rate spread)
                             delete-Instrument))
          (let ((fair-spread (SimpleSwap-fair-spread swap)))
            (deleting-let ((swap (make-test-swap today length rate fair-spread)
                                 delete-Instrument))
              (check-expected (Instrument-NPV swap) 0.0 1.0e-10
  ...
  (deleting-let ((today (Date-todays-date) delete-Date))
    (for-each-combination ((length '(1 2 5 10 20))
      ...
      (let ((values (map (lambda (r)
                           (deleting-let ((swap (make-test-swap today length 
                                                                r spread)
                                                delete-Instrument))
                             (Instrument-NPV swap)))
...
(define (SimpleSwap-cached-value-test)
  (deleting-let* ((today (new-Date 17 6 2002) delete-Date)
                  (swap (make-test-swap today 10 0.06 0.001) 
                        delete-Instrument))
