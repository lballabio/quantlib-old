...
(define (Implied-term-structure-consistency-test)
  (deleting-let* ((term-structure (make-test-structure-handle)
                  ...
                  (new-settlement (Date-plus-years 
    ...
    (let ((tolerance 1.0e-10)
          (base-discount (TermStructureHandle-discount term-structure
                                                       new-settlement))
          (discount (TermStructureHandle-discount term-structure test-date))
  ...
  (let ((flag #f))
    (deleting-let* ((term-structure (make-test-structure-handle)
      ...
      (deleting-let ((new-term-structure (new-FlatForward settlement
                                                          0.05 day-counter)
                                         delete-TermStructure))
        (TermStructureHandle-link-to! term-structure new-term-structure))
      (if (not flag)
...
(define (Forward-spreaded-term-structure-consistency-test)
  (deleting-let* ((term-structure (make-test-structure-handle)
                  ...
                  (test-date (Date-plus-years 
                              ...
                              5)
                             delete-Date)
                  (spreaded (new-ForwardSpreadedTermStructure term-structure h)
                            delete-TermStructure))
    (let ((tolerance 1.0e-10)
          (forward (TermStructureHandle-instantaneous-forward term-structure 
                                                              test-date))
...
(define (Forward-spreaded-term-structure-observability-test)
  (let ((flag #f)
    ...
    (deleting-let* ((term-structure (make-test-structure-handle)
                    ...
                    (h (new-MarketElementHandle me)
                       delete-MarketElementHandle)
                    (spreaded (new-ForwardSpreadedTermStructure term-structure 
                                                                h)
                              delete-TermStructure)
                    (obs (new-Observer (lambda () (set! flag #t)))
                         delete-Observer))
...
(define (Zero-spreaded-term-structure-consistency-test)
  (deleting-let* ((term-structure (make-test-structure-handle)
                  ...
                  (test-date (Date-plus-years 
                              ...
                              5)
                             delete-Date)
                  (spreaded (new-ZeroSpreadedTermStructure term-structure h)
                            delete-TermStructure))
    (let ((tolerance 1.0e-10)
          (zero (TermStructureHandle-zero-yield term-structure test-date))
