...
%rename(__len__) Path::size;
#elif defined(SWIGMZSCHEME) || defined(SWIGGUILE)
%rename(length)  Path::size;
#endif
class Path {
  private:
    Path();
...
#if defined(SWIGPYTHON) || defined(SWIGRUBY)
%rename(__len__) MultiPath::pathSize;
#elif defined(SWIGMZSCHEME) || defined(SWIGGUILE)
%rename("length")       MultiPath::pathSize;
%rename("asset-number") MultiPath::assetNumber;
#endif
class MultiPath {
  private:
    MultiPath();
