               ...
               (let ((engines '("jr""crr" "eqp" "trigeorgis" "tian")))
                 (deleting-let* ((ref-option (make-option type 
                   ...
                   (let ((ref-value (Instrument-NPV ref-option)))
                     (for-each
                      (lambda (entry)
                        (check-equal ref-value (Instrument-NPV (cdr entry))
                                     (* u tolerance)
                                     "Option parameters: "
                                     type ", " u ", " strike ", " q ", "
                                     r ", " ex-days ", " v cr
...
(define EuropeanOption-suite
  (make-test-suite 
   "European option tests"
   (make-test-case/msg "Testing European option greeks"
                       (European-option-Greek-test))
   (make-test-case/msg "Testing European option implied volatility"
                       (European-option-implied-vol-test))
   (make-test-case/msg 
    "Testing binomial European engines against analytic results"
    (European-option-binomial-engine-test))))
