    ...
    (let ((methods (list 
      ...
      (let ((cases
             '((McEuropean "Put" 80.0 85.0 -0.03 0.05
                           0.25 0.2 5.0394064744 #f)
               (McEuropean "Put" 80.0 85.0 -0.03 0.05
                           0.25 0.2 5.2783973809 #t)
               (McEuropean "Call" 80.0 85.0 -0.03 0.05 
                           0.25 0.2 2.0523055758 #f)
               (McEuropean "Call" 80.0 85.0 -0.03 0.05 
                           0.25 0.2 1.9590724337 #t)
               (McEuropean "Straddle" 80.0 85.0 -0.03 0.05 
                           0.25 0.2 7.0917120502 #f)
               (McEuropean "Straddle" 80.0 85.0 -0.03 0.05
                           0.25 0.2 7.2374698146 #t))))
        (for-each-case ((tag optionType underlying strike
                             dividendYield riskFreeRate residualTime
            ...
            (let-at-once ((_ make delete value value/samples error-estimate) 
              ...
              (deleting-let ((p (make optionType underlying strike 
                ...
                (let* ((pvalue (value/samples p fixedSamples))
                       ...
                       (pvalue2 (value p tol))
                       (accuracy (/ (error-estimate p) pvalue2)))
                  (check-expected pvalue storedValue 2.0e-2
                                  "In batch 5: ")
                  (check (<= accuracy tol)
                         "In batch 5:" cr
                         "    accuracy reached    : " accuracy cr
...
(define (OldMcMultiFactorPricers-test)
  (let* ((cor #(#(1.00 0.50 0.30 0.10)
         ...
         (cov (get-covariance volatilities cor))
         ...
         (sameAssetCovariance (get-covariance sameAssetVols
                                              perfectCorrelation))
    ...
    ; McEverest
    (for-each-case ((antithetic? storedValue) '((#f 0.7562173725)
                                                (#t 0.75899732)))
    ...
    (let ((sameAssetValues #(25 25 25 25))
          (type "Call")
          (strike 100))
      (for-each-case ((antithetic? storedValue) '((#f 12.5780178807)
                                                  (#t 12.958451866)))
    ...
    (let ((assetValues #(100 110 90 105)))
      (for-each-case ((antithetic? storedValue) '((#f 122.703208891)
                                                  (#t 123.3033777758)))
    ...
    (let ((portfolio #(0.15 0.20 0.35 0.30))
      ...
      (for-each-case ((antithetic? storedValue) '((#f 0.0380331503)
                                                  (#t 0.0402704285)))
    ...
    (let ((assetValues #(100 110 90 105))
          (timeIncrements '(0.25 0.5 0.75 1))
          (strike 101))
      (for-each-case ((antithetic? storedValue) '((#f 5.782701442)
                                                  (#t 6.0621755931)))
...
(define OldPricers-suite
  (make-test-suite 
   "old tests"
   (make-test-case/msg "Testing old-style barrier option pricer"
                       (OldBarrierPricer-test))
   (make-test-case/msg "Testing old-style binary option pricer"
                       (OldBinaryPricer-test))
   (make-test-case/msg "Testing old-style cliquet option pricer"
                       (OldCliquetPricer-test))
   (make-test-case/msg 
    "Testing old-style European option pricer with dividends"
    (OldDividendEuropeanPricer-test))
   (make-test-case/msg 
    "Testing old-style finite-difference European option pricer"
    (OldFdEuropeanPricer-test))
   (make-test-case/msg "Testing old-style American-type pricers"
                       (OldAmericanPricers-test))
   (make-test-case/msg "Testing old-style Monte Carlo single-factor pricers"
                       (OldMcSingleFactorPricers-test))
   (make-test-case/msg "Testing old-style Monte Carlo multi-factor pricers"
                       (OldMcMultiFactorPricers-test))))
