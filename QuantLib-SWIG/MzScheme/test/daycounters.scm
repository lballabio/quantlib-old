...
(define (Day-counter-test-isda isda d1 d2 expected)
  (let ((calculated (DayCounter-year-fraction isda d1 d2)))
	(if (not (<= (abs (- calculated expected)) 1.0e-10))
		(let ((error-msg
			   (string-append
				(format "ISDA day counter:~n")
				(format "    first date:       ~a~n" (Date->string d1))
				(format "    second date:      ~a~n" (Date->string d2))
				(format "    expected value:   ~a~n" expected)
				(format "    calculated value: ~a~n" calculated))))
		  (error error-msg)))))
...
(define (Day-counter-test-isma isma d1 d2 start-ref end-ref expected)
  (let ((calculated (DayCounter-year-fraction isma d1 d2 start-ref end-ref)))
	(if (not (<= (abs (- calculated expected)) 1.0e-10))
		(let ((error-msg
			   (string-append
				(format "ISMA day counter:~n")
				(format "    first date:       ~a~n" (Date->string d1))
				(format "    second date:      ~a~n" (Date->string d2))
				(format "    first ref. date:  ~a~n" (Date->string start-ref))
				(format "    second ref. date: ~a~n" (Date->string end-ref))
				(format "    expected value:   ~a~n" expected)
				(format "    calculated value: ~a~n" calculated))))
		  (error error-msg)))))
