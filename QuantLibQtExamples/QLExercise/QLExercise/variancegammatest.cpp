...
void VarianceGammaTest::testVarianceGamma() {
	...
	BOOST_MESSAGE("Testing Variance Gamma model for European options...");
	...
	for (Size i=0; i<LENGTH(processes); i++) {
		...
		boost::shared_ptr<FFTVarianceGammaEngine> fftEngine(
			new FFTVarianceGammaEngine(stochProcess));//Å°

		// which requires a list of options
		std::vector<boost::shared_ptr<Instrument> > optionList;//Å°

		std::vector<boost::shared_ptr<StrikedTypePayoff> > payoffs;//Å°
		...
		for (Size j=0; j<LENGTH(options); j++)
		{
			...
			std::cout << "Option Price (FFT): " << option->NPV() << std::endl;
		}
	}
}