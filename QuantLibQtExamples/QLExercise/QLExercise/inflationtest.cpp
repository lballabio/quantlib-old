...
//===========================================================================================
// zero inflation tests, index, term structure, and swaps
//===========================================================================================
...
void InflationTest::testZeroIndex() {
	...
	Date todayMinusLag = evaluationDate - iir->availabilityLag(); std::cout << todayMinusLag << std::endl;
	std::pair<Date,Date> lim = inflationPeriod(todayMinusLag, iir->frequency());
	todayMinusLag = lim.first; std::cout << todayMinusLag << std::endl;
	std::cout << "Inflation Period 1: " << lim.first << "\t" << "Inflation Period 2: " << lim.second << std::endl;

	Real eps = 1.0e-8;

	
	Size widths[] = { 13, 30 };	
	std::cout << std::setw(widths[0]) << std::left << "CPI"
			  << std::setw(widths[1]) << std::left << "Date"
			  << std::endl;
	...
	for (Size i=0; i<rpiSchedule.size()-1;i++) {
		...
		std::cout << std::setw(widths[0]) << std::left << fixData[i]
			      << std::left << lim.first
			      << std::endl;
	}
	//std::cout << LENGTH(fixData) << std::endl; //32
	//std::cout << rpiSchedule.size() << std::endl; //33
}
...
void InflationTest::testZeroTermStructure() {
	...
	RelinkableHandle<ZeroInflationTermStructure> hz;//Å°KEYÅ°
	bool interp = false;
	boost::shared_ptr<UKRPI> iiUKRPI(new UKRPI(interp, hz));//Å°KEYÅ°
	for (Size i=0; i<LENGTH(fixData); i++) {
		iiUKRPI->addFixing(rpiSchedule[i], fixData[i]);
	}

	boost::shared_ptr<ZeroInflationIndex> ii = boost::dynamic_pointer_cast<ZeroInflationIndex>(iiUKRPI); //Å°
	boost::shared_ptr<YieldTermStructure> nominalTS = nominalTermStructure(); //Å°
	...
	boost::shared_ptr<PiecewiseZeroInflationCurve<Linear> > pZITS(
						...
						Handle<YieldTermStructure>(nominalTS), helpers));//Å°KEYÅ°
	pZITS->recalculate();//Å°KEYÅ°


	std::cout << "Date" << "\tZero Rate" << "\tImplied Rate" << std::endl;
	...
	for (Size i=0; i<LENGTH(zcData); i++) {
		BOOST_REQUIRE_MESSAGE(std::fabs(zcData[i].rate/100.0
			- pZITS->zeroRate(zcData[i].date, observationLag, forceLinearInterpolation)) < eps,//Å°KEYÅ°
	...
	Real correctIndexed = ii->fixing(iicf.fixingDate())/ii->fixing(iicf.baseDate());//Å°KEYÅ°
	Real calculatedIndexed = iicf.amount()/iicf.notional();//Å°KEYÅ°
	...
	boost::shared_ptr<PricingEngine> sppe(new DiscountingSwapEngine(hTS));//Å°KEYÅ°
	nzcis.setPricingEngine(sppe);//Å°KEYÅ°

	// ... and price it, should be zero
	BOOST_CHECK_MESSAGE(fabs(nzcis.NPV())<0.00001,"ZCIS does not reprice to zero "//Å°KEYÅ°
						<< nzcis.NPV()//Å°KEYÅ°
						<< evaluationDate << " to " << zcData[6].date << " becoming " << nzcis.maturityDate()//Å°KEYÅ°
						<< " rate " << zcData[6].rate
						<< " fixed leg " << nzcis.legNPV(0)//Å°KEYÅ°
						<< " indexed-predicted inflated leg " << nzcis.legNPV(1)//Å°KEYÅ°
						<< " discount " << nominalTS->discount(nzcis.maturityDate())//Å°KEYÅ°
						);
	...
	//
	boost::shared_ptr<MultiplicativePriceSeasonality> seasonality_1(new MultiplicativePriceSeasonality());//Å°KEYÅ°
	std::vector<Rate> seasonalityFactors_1(12, 1.0);
	seasonality_1->set(seasonallityBaseDate,Monthly,seasonalityFactors_1);//Å°KEYÅ°

	boost::shared_ptr<MultiplicativePriceSeasonality> seasonality_real(
		new MultiplicativePriceSeasonality(seasonallityBaseDate,Monthly,seasonalityFactors));//Å°KEYÅ°
	...
	hz->setSeasonality(seasonality_1);//Å°KEYÅ°
	...
	for (Size i=0; i<LENGTH(zcData); i++) {
		BOOST_CHECK_MESSAGE(std::fabs(zcData[i].rate/100.0
					...
					<< " forceLinearInterpolation " << forceLinearInterpolation
					<< " diff= " << (zcData[i].rate/100.0 - pZITSyes->zeroRate(zcData[i].date, observationLagyes, forceLinearInterpolation)));
		BOOST_CHECK_MESSAGE(std::fabs(helpersyes[i]->impliedQuote()
					...
					<< " vs " << zcData[i].rate/100.0
					<< " diff= " << (helpersyes[i]->impliedQuote() - zcData[i].rate/100.0));
	}
...
void InflationTest::testYYTermStructure() {
	...
	RelinkableHandle<YoYInflationTermStructure> hy; //Å°Å°Å°
	bool interp = false;
	boost::shared_ptr<YYUKRPIr> iir(new YYUKRPIr(interp, hy)); //Å°Å°Å°
	for (Size i=0; i<LENGTH(fixData); i++) {
		iir->addFixing(rpiSchedule[i], fixData[i]);
	}
	...
	boost::shared_ptr<YieldTermStructure> nominalTS = nominalTermStructure(); //Å°Å°Å°
	...
	boost::shared_ptr<PiecewiseYoYInflationCurve<Linear> > pYYTS(
		new PiecewiseYoYInflationCurve<Linear>(
				...
				Handle<YieldTermStructure>(nominalTS), helpers)); //Å°Å°Å°
	pYYTS->recalculate(); //Å°Å°Å°
	...
	Real eps = 0.000001;
	// usual swap engine
	Handle<YieldTermStructure> hTS(nominalTS); //Å°Å°Å°
	boost::shared_ptr<PricingEngine> sppe(new DiscountingSwapEngine(hTS)); //Å°Å°Å°

	// make sure that the index has the latest yoy term structure
	hy.linkTo(pYYTS); //Å°Å°Å°
	...
	for (Size j = 1; j < LENGTH(yyData); j++) {
		...
		yyS2.setPricingEngine(sppe); //Å°Å°Å°

		//std::cout << "YoY Swap 2 " << j << " NPV= " << yyS2.NPV() << std::endl;
		BOOST_CHECK_MESSAGE(fabs(yyS2.NPV())<eps,"fresh yoy swap NPV!=0 from TS "  //Å°Å°Å° yyS2.NPV()
				<<"swap quote for pt " << j
	...
	for (Size k = 0; k < 14; k++) {
		...
		BOOST_CHECK_MESSAGE(fabs(yyS3.NPV())< 20000.0,
							"yyS3: unexpected size of aged YoY swap, aged "
...
void InflationTest::myInflationTest(){
