...
#define REPORT_FAILURE(greekName, exercise, \
    ...
    std::cout << exerciseTypeToString(exercise) << " " \
...
void MargrabeOptionTest::testEuroExchangeTwoAssets() {
    ...
    std::cout << "\nTesting European one-asset-for-another option..." << std::endl;
...
void MargrabeOptionTest::testGreeks() {
    ...
    std::cout << "\nTesting analytic European exchange option greeks..." << std::endl;
    ...
    for (Size i=0; i<LENGTH(types); i++) {
        for (Size k=0; k<LENGTH(residualTimes); k++) {
          ...
          MargrabeOption margrabeOption(1, 1, exercise); ///// KEY //////

          // analytic engine
          margrabeOption.setPricingEngine(engine); ///// KEY //////
                  ...
                  for (Size n=0; n<LENGTH(rRates); n++) {
                    for (Size p=0; p<LENGTH(vols1); p++) {
                      ...
                      if (value > spot1->value()*1.0e-5) {
                          ...
                          Real du = u*1.0e-4;
                          spot1->setValue(u+du);
                          Real value_p = margrabeOption.NPV(), ///// KEY //////
...
void MargrabeOptionTest::testAmericanExchangeTwoAssets() {
    ...
    std::cout << "\nTesting American one-asset-for-another option..." << std::endl;
...
void MargrabeOptionTest::simpleMargrabeOption()
{
	std::cout << "\nTesting quite a simple Margrabe European option..." << std::endl;
	
	DayCounter dc = Actual360();
	Date today = Settings::instance().evaluationDate();

	boost::shared_ptr<SimpleQuote> spot1(new SimpleQuote(100.0));
	boost::shared_ptr<SimpleQuote> spot2(new SimpleQuote(93.0));

	boost::shared_ptr<SimpleQuote> qRate1(new SimpleQuote(0.032));
	boost::shared_ptr<YieldTermStructure> qTS1 = flatRate(today, qRate1, dc);
	boost::shared_ptr<SimpleQuote> qRate2(new SimpleQuote(0.009));
	boost::shared_ptr<YieldTermStructure> qTS2 = flatRate(today, qRate2, dc);

	boost::shared_ptr<SimpleQuote> rRate(new SimpleQuote(0.018));
	boost::shared_ptr<YieldTermStructure> rTS = flatRate(today, rRate, dc);

	boost::shared_ptr<SimpleQuote> vol1(new SimpleQuote(0.25));
	boost::shared_ptr<BlackVolTermStructure> volTS1 = flatVol(today, vol1, dc);
	boost::shared_ptr<SimpleQuote> vol2(new SimpleQuote(0.05));
	boost::shared_ptr<BlackVolTermStructure> volTS2 = flatVol(today, vol2, dc);
	
	Date exDate = today + Integer(0.1*360+0.5);
	boost::shared_ptr<Exercise> exercise(new EuropeanExercise(exDate));
	...
	std::cout << "Margrabe Option NPV: " << value_p << std::endl;
}
