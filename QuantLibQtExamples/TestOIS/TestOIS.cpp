...
int main() {
    ...
    /*
    OvernightIndexedSwapTest oistest;
    oistest.testFairRate();
    oistest.testFairSpread();
    oistest.testCachedValue();
    oistest.testBootstrap();
    */
    ...
    */
    ...
    // test curve consistency
    std::cout << "test curve consistency" << std::endl;
    std::cout << "swap length\tquoted\tcalculated\terror\ttolerance" << std::endl;
    Real tolerance = 1.0e-8;
    for (Size i = 0; i < LENGTH(eoniaSwapData); i++) {
        Rate expected = eoniaSwapData[i].rate;
        Period term = eoniaSwapData[i].n * eoniaSwapData[i].unit;
        ...
        /*
        std::cout << std::setw(2) << i << ": curve inconsistency:" << std::setprecision(10) <<
                        "\n swap length:     " << term <<
                        "\n quoted rate:     " << expected <<
                        "\n calculated rate: " << calculated <<
                        "\n error:           " << error <<
                        "\n tolerance:       " << tolerance << std::endl << std::endl;
                        */
        std::cout << std::setw(2) << i << std::setprecision(10) <<
            "\t" << term <<
            "\t" << expected <<
            "\t" << calculated <<
            "\t" << error <<
            "\t" << tolerance << std::endl << std::endl;
    }
    ...
    /*
    OvernightIndex oi;
    OvernightIndexedSwapIndex oisi;
    OvernightIndexedSwap ois;
    OISRateHelper oisRateHelper;
    OISRateHelper (Natural settlementDays, const Period &tenor, const Handle< Quote > &fixedRate, 
                    const boost::shared_ptr< OvernightIndex > &overnightIndex)
    OvernightIndex (const std::string &familyName, Natural settlementDays, const Currency &currency, 
