...
int main(int, char* []) {
    ...
    try {
         ...
         *********************/
         ...
         // (3) Floating rate bond (3M USD Libor + 0.1%)
         ...
         // (4) Plain Mortgage Backed Securities

         Schedule mbsSchedule(Date(15, May, 2007),
                 Date(15,May,2017), Period(Semiannual),
                 UnitedStates(UnitedStates::GovernmentBond),
                 Unadjusted, Unadjusted, DateGeneration::Backward, false);
         ...
         boost::shared_ptr<PricingEngine> mbsEngine(
                 new DiscountingBondEngine(discountingTermStructure));

         mbs.setPricingEngine(mbsEngine);
