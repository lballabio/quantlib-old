...
class ConvexMonotoneImpl : public Interpolation::templateImpl<I1, I2> {
  ...
  public:
    ...
    ConvexMonotoneImpl(const I1 &xBegin, const I1 &xEnd, const I2 &yBegin,
                       Real quadraticity, Real monotonicity, bool forcePositive,
        ...
        : Interpolation::templateImpl<I1, I2>(xBegin, xEnd, yBegin),
          ...
          constantLastPeriod_(constantLastPeriod), quadraticity_(quadraticity),
          monotonicity_(monotonicity), length_(xEnd - xBegin) {
...
class ComboHelper : public SectionHelper {
  public:
    ComboHelper(boost::shared_ptr<SectionHelper> &quadraticHelper,
                ...
                Real quadraticity)
        : quadraticity_(quadraticity), quadraticHelper_(quadraticHelper),
    ...
    Real value(Real x) const {
        return (quadraticity_ * quadraticHelper_->value(x) +
                (1.0 - quadraticity_) * convMonoHelper_->value(x));
    }
    Real primitive(Real x) const {
    ...
    Real fNext() const {
        return (quadraticity_ * quadraticHelper_->fNext() +
                (1.0 - quadraticity_) * convMonoHelper_->fNext());
    }
...
class EverywhereConstantHelper : public SectionHelper {
  public:
    ...
    Real value(Real) const { return value_; }
    Real primitive(Real x) const {
        return prevPrimitive_ + (x - xPrev_) * value_;
    }
    Real fNext() const { return value_; }
...
class ConvexMonotone2Helper : public SectionHelper {
  public:
    ConvexMonotone2Helper(Real xPrev, Real xNext, Real gPrev, Real gNext,
                          Real fAverage, Real eta2, Real prevPrimitive)
        : xPrev_(xPrev), xScaling_(xNext - xPrev), gPrev_(gPrev), gNext_(gNext),
          fAverage_(fAverage), eta2_(eta2), prevPrimitive_(prevPrimitive) {}
    ...
    Real primitive(Real x) const {
        ...
        if (xVal <= eta2_) {
            ...
            return (prevPrimitive_ +
                    xScaling_ *
                        (fAverage_ * xVal + gPrev_ * xVal +
                         (gNext_ - gPrev_) / ((1 - eta2_) * (1 - eta2_)) *
                             (1.0 / 3.0 *
                                  (xVal * xVal * xVal - eta2_ * eta2_ * eta2_) -
                              eta2_ * xVal * xVal + eta2_ * eta2_ * xVal)));
        }
    }
    Real fNext() const { return (fAverage_ + gNext_); }
...
class ConvexMonotone3Helper : public SectionHelper {
  public:
    ConvexMonotone3Helper(Real xPrev, Real xNext, Real gPrev, Real gNext,
                          Real fAverage, Real eta3, Real prevPrimitive)
        : xPrev_(xPrev), xScaling_(xNext - xPrev), gPrev_(gPrev), gNext_(gNext),
          fAverage_(fAverage), eta3_(eta3), prevPrimitive_(prevPrimitive) {}

    Real value(Real x) const {
        Real xVal = (x - xPrev_) / xScaling_;
        if (xVal <= eta3_) {
            return (fAverage_ + gNext_ +
                    (gPrev_ - gNext_) / (eta3_ * eta3_) * (eta3_ - xVal) *
                        (eta3_ - xVal));
        } else {
            return (fAverage_ + gNext_);
        }
    ...
    Real primitive(Real x) const {
        ...
        if (xVal <= eta3_) {
            return (prevPrimitive_ +
                    xScaling_ *
                        (fAverage_ * xVal + gNext_ * xVal +
                         (gPrev_ - gNext_) / (eta3_ * eta3_) *
                             (1.0 / 3.0 * xVal * xVal * xVal -
                              eta3_ * xVal * xVal + eta3_ * eta3_ * xVal)));
        } else {
            return (prevPrimitive_ +
...
class ConvexMonotone4Helper : public SectionHelper {
  public:
    ConvexMonotone4Helper(Real xPrev, Real xNext, Real gPrev, Real gNext,
                          Real fAverage, Real eta4, Real prevPrimitive)
        : xPrev_(xPrev), xScaling_(xNext - xPrev), gPrev_(gPrev), gNext_(gNext),
          fAverage_(fAverage), eta4_(eta4), prevPrimitive_(prevPrimitive) {
    ...
    Real value(Real x) const {
        ...
        if (xVal <= eta4_) {
            return (fAverage_ + A_ +
                    (gPrev_ - A_) * (eta4_ - xVal) * (eta4_ - xVal) /
                        (eta4_ * eta4_));
        } else {
            return (fAverage_ + A_ +
                    (gNext_ - A_) * (xVal - eta4_) * (xVal - eta4_) /
                        ((1 - eta4_) * (1 - eta4_)));
        }
    }
    ...
    Real primitive(Real x) const {
        ...
        if (xVal <= eta4_) {
            retVal = prevPrimitive_ +
                     xScaling_ * (fAverage_ + A_ +
                                  (gPrev_ - A_) / (eta4_ * eta4_) *
                                      (eta4_ * eta4_ - eta4_ * xVal +
                                       1.0 / 3.0 * xVal * xVal)) *
                         xVal;
        } else {
            retVal =
                prevPrimitive_ +
                xScaling_ * (fAverage_ * xVal + A_ * xVal +
                             (gPrev_ - A_) * (1.0 / 3.0 * eta4_) +
                             (gNext_ - A_) / ((1 - eta4_) * (1 - eta4_)) *
                                 (1.0 / 3.0 * xVal * xVal * xVal -
                                  eta4_ * xVal * xVal + eta4_ * eta4_ * xVal -
...
class ConvexMonotone4MinHelper : public ConvexMonotone4Helper {
  public:
    ConvexMonotone4MinHelper(Real xPrev, Real xNext, Real gPrev, Real gNext,
                             Real fAverage, Real eta4, Real prevPrimitive)
        : ConvexMonotone4Helper(xPrev, xNext, gPrev, gNext, fAverage, eta4,
                                prevPrimitive),
          splitRegion_(false) {
        if (A_ + fAverage_ <= 0.0) {
            splitRegion_ = true;
            ...
            Real reqdShift =
                (eta4_ * fPrev + (1 - eta4_) * fNext) / 3.0 - fAverage_;
    ...
    Real primitive(Real x) const {
        ...
        if (x <= x2_) {
            xVal /= xRatio_;
            return (prevPrimitive_ +
                    xScaling_ * xRatio_ * (fAverage_ + A_ +
                                           (gPrev_ - A_) / (eta4_ * eta4_) *
                                               (eta4_ * eta4_ - eta4_ * xVal +
                                                1.0 / 3.0 * xVal * xVal)) *
                        xVal);
...
class ConstantGradHelper : public SectionHelper {
  public:
    ConstantGradHelper(Real fPrev, Real prevPrimitive, Real xPrev, Real xNext,
                       Real fNext)
        : fPrev_(fPrev), prevPrimitive_(prevPrimitive), xPrev_(xPrev),
          fGrad_((fNext - fPrev) / (xNext - xPrev)), fNext_(fNext) {}

    Real value(Real x) const { return (fPrev_ + (x - xPrev_) * fGrad_); }
    Real primitive(Real x) const {
        return (prevPrimitive_ +
                (x - xPrev_) * (fPrev_ + 0.5 * (x - xPrev_) * fGrad_));
    }
    Real fNext() const { return fNext_; }
...
class QuadraticHelper : public SectionHelper {
  public:
    QuadraticHelper(Real xPrev, Real xNext, Real fPrev, Real fNext,
                    Real fAverage, Real prevPrimitive)
        : xPrev_(xPrev), xNext_(xNext), fPrev_(fPrev), fNext_(fNext),
          fAverage_(fAverage), prevPrimitive_(prevPrimitive) {
    ...
    Real primitive(Real x) const {
        Real xVal = (x - xPrev_) / xScaling_;
        return (prevPrimitive_ +
                xScaling_ * (a_ / 3 * xVal * xVal + b_ / 2 * xVal + c_) * xVal);
    }
...
class QuadraticMinHelper : public SectionHelper {
  public:
    QuadraticMinHelper(Real xPrev, Real xNext, Real fPrev, Real fNext,
                       Real fAverage, Real prevPrimitive)
        : splitRegion_(false), x1_(xPrev), x4_(xNext),
          primitive1_(prevPrimitive), fAverage_(fAverage), fPrev_(fPrev),
          fNext_(fNext) {
        a_ = 3 * fPrev_ + 3 * fNext_ - 6 * fAverage_;
        b_ = -(4 * fPrev_ + 2 * fNext_ - 6 * fAverage_);
        c_ = fPrev_;
        ...
        if (d > 0) {
            ...
            Real cAv =
                4 * (fPrev_ * fPrev_ + fPrev_ * fNext_ + fNext_ * fNext_);
            ...
            if (dAv >= 0.0) {
                ...
                primitive2_ =
                    primitive1_ +
                    xScaling_ * (a_ / 3 * xRoot * xRoot + b_ / 2 * xRoot + c_) *
                        xRoot;
            }
        }
    }
...
template <class I1, class I2> void ConvexMonotoneImpl<I1, I2>::update() {
    sectionHelpers_.clear();
    if (length_ == 2) { // single period
        boost::shared_ptr<SectionHelper> singleHelper(
            new EverywhereConstantHelper(this->yBegin_[1], 0.0,
                                         this->xBegin_[0]));
    ...
    for (Size i = startPoint; i < endPoint; ++i) {
        ...
        if (std::fabs(gPrev) < 1.0E-14 && std::fabs(gNext) < 1.0E-14) {
            ...
            if (quadraticity_ > 0.0) {
                if (gPrev >= -2.0 * gNext && gPrev > -0.5 * gNext &&
                    forcePositive_) {
                    quadraticHelper =
                        boost::shared_ptr<SectionHelper>(new QuadraticMinHelper(
                            this->xBegin_[i - 1], this->xBegin_[i], f[i - 1],
                            f[i], this->yBegin_[i], primitive));
                } else {
                    quadraticHelper =
                        boost::shared_ptr<SectionHelper>(new QuadraticHelper(
                            this->xBegin_[i - 1], this->xBegin_[i], f[i - 1],
                            f[i], this->yBegin_[i], primitive));
                }
            }
            if (quadraticity_ < 1.0) {
                ...
                if ((gPrev > 0.0 && -0.5 * gPrev >= gNext &&
                     gNext >= -2.0 * gPrev) ||
                    (gPrev < 0.0 && -0.5 * gPrev <= gNext &&
                     gNext <= -2.0 * gPrev)) {
                    quadraticity = 1.0;
                    if (quadraticity_ == 0) {
                        if (forcePositive_) {
                            quadraticHelper = boost::shared_ptr<SectionHelper>(
                                new QuadraticMinHelper(
                                    this->xBegin_[i - 1], this->xBegin_[i],
                                    f[i - 1], f[i], this->yBegin_[i],
                                    primitive));
                        } else {
                            quadraticHelper = boost::shared_ptr<SectionHelper>(
                                new QuadraticHelper(this->xBegin_[i - 1],
                                                    this->xBegin_[i], f[i - 1],
                                                    f[i], this->yBegin_[i],
                                                    primitive));
                        }
                    }
                    ...
                    if (eta < b2) {
                        convMonotoneHelper = boost::shared_ptr<SectionHelper>(
                            new ConvexMonotone2Helper(
                                this->xBegin_[i - 1], this->xBegin_[i], gPrev,
                                gNext, this->yBegin_[i], eta, primitive));
                    } else {
                        if (forcePositive_) {
                            convMonotoneHelper =
            ...
            if (quadraticity == 1.0) {
                ...
                sectionHelpers_[this->xBegin_[i]] =
                    boost::shared_ptr<SectionHelper>(new ComboHelper(
                        quadraticHelper, convMonotoneHelper, quadraticity));
            }
        }
        primitive +=
    ...
    if (constantLastPeriod_) {
        sectionHelpers_[this->xBegin_[length_ - 1]] =
            boost::shared_ptr<SectionHelper>(new EverywhereConstantHelper(
                this->yBegin_[length_ - 1], primitive,
...
class ConvexMonotoneInterpolation : public Interpolation {
  ...
  public:
    ConvexMonotoneInterpolation(
        const I1 &xBegin, const I1 &xEnd, const I2 &yBegin, Real quadraticity,
        Real monotonicity, bool forcePositive, bool flatFinalPeriod = false,
        const helper_map &preExistingHelpers = helper_map()) {
        impl_ = boost::shared_ptr<Interpolation::Impl>(
            new detail::ConvexMonotoneImpl<I1, I2>(
                xBegin, xEnd, yBegin, quadraticity, monotonicity, forcePositive,
...
template <class T = Real> class ConvexMonotone {
  public:
    static const bool global = true;
    static const Size requiredPoints = 2;
    static const Size dataSizeAdjustment = 1;

    explicit ConvexMonotone(Real quadraticity = 0.3, Real monotonicity = 0.7,
        ...
        if (length - localisation == 1) { // the first time this
            ...
            if (length == finalSize) {
                return ConvexMonotoneInterpolation<I1, I2>(
                    xBegin, xEnd, yBegin, quadraticity_, monotonicity_,
                    forcePositive_, false);
            } else {
                return ConvexMonotoneInterpolation<I1, I2>(
                    xBegin, xEnd, yBegin, quadraticity_, monotonicity_,
