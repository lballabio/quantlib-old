...
namespace QuantLib {
	...
	template<class T>
    class DiscountingBondEngine_t : public Bond_t<T>::engine {
      public:
	...
	template<class T>
	void DiscountingBondEngine_t<T>::calculate() const {
		...
		this->results_.value = CashFlows::npv<T>(this->arguments_.cashflows,
			**discountCurve_,
			includeRefDateFlows,
			this->results_.valuationDate,
			this->results_.valuationDate);
		...
		if (!includeRefDateFlows
			...
			// no such luck
			this->results_.settlementValue =
				CashFlows::npv<T>(this->arguments_.cashflows,
				**(this->discountCurve_),
				false,
				this->arguments_.settlementDate,
				this->arguments_.settlementDate);
		}
	}
}
