...
struct BondFunctions {
    ...
    previousCashFlow(const Bond &bond, Date refDate = Date()) {
    ...
    inline static Rate atmRate(const Bond &bond,
        ...
        return impl::atmRate<Real>(bond, discountCurve, settlementDate,
                                   cleanPrice);
    }
    ...
    //@{
    inline static Real cleanPrice(const Bond &bond, const InterestRate &yield,
                                  Date settlementDate = Date()) {
        return impl::cleanPrice<Real>(bond, yield, settlementDate);
    }
    inline static Real cleanPrice(const Bond &bond, Rate yield,
                                  const DayCounter &dayCounter,
                                  Compounding compounding, Frequency frequency,
                                  Date settlementDate = Date()) {
        return impl::cleanPrice<Real>(bond, yield, dayCounter, compounding,
                                      frequency, settlementDate);
    }
    inline static Real dirtyPrice(const Bond &bond, const InterestRate &yield,
                                  Date settlementDate = Date()) {
        return impl::dirtyPrice<Real>(bond, yield, settlementDate);
    }
    inline static Real dirtyPrice(const Bond &bond, Rate yield,
                                  const DayCounter &dayCounter,
                                  Compounding compounding, Frequency frequency,
                                  Date settlementDate = Date()) {
        return impl::dirtyPrice<Real>(bond, yield, dayCounter, compounding,
                                      frequency, settlementDate);
    }
    inline static Real bps(const Bond &bond, const InterestRate &yield,
                           Date settlementDate = Date()) {
        return impl::bps<Real>(bond, yield, settlementDate);
    }
    inline static Real bps(const Bond &bond, Rate yield,
                           const DayCounter &dayCounter,
                           Compounding compounding, Frequency frequency,
                           Date settlementDate = Date()) {
        return impl::bps<Real>(bond, yield, dayCounter, compounding, frequency,
                               settlementDate);
    }
    inline static Rate yield(const Bond &bond, Real cleanPrice,
                             const DayCounter &dayCounter,
                             Compounding compounding, Frequency frequency,
                             Date settlementDate = Date(),
                             Real accuracy = 1.0e-10, Size maxIterations = 100,
                             Rate guess = 0.05) {
        return impl::yield<Real>(bond, cleanPrice, dayCounter, compounding,
                                 frequency, settlementDate, accuracy,
                                 maxIterations, guess);
    }
    inline static Time duration(const Bond &bond, const InterestRate &yield,
                                Duration::Type type = Duration::Modified,
                                Date settlementDate = Date()) {
        return impl::duration<Real>(bond, yield, type, settlementDate);
    }
    inline static Time duration(const Bond &bond, Rate yield,
                                const DayCounter &dayCounter,
                                Compounding compounding, Frequency frequency,
                                Duration::Type type = Duration::Modified,
                                Date settlementDate = Date()) {
        return impl::duration<Real>(bond, yield, dayCounter, compounding,
                                    frequency, type, settlementDate);
    }
    inline static Real convexity(const Bond &bond, const InterestRate &yield,
                                 Date settlementDate = Date()) {
        return impl::convexity<Real>(bond, yield, settlementDate);
    }
    inline static Real convexity(const Bond &bond, Rate yield,
                                 const DayCounter &dayCounter,
                                 Compounding compounding, Frequency frequency,
                                 Date settlementDate = Date()) {
        return impl::convexity<Real>(bond, yield, dayCounter, compounding,
                                     frequency, settlementDate);
    }
    ...
    inline static Real basisPointValue(const Bond &bond, Rate yield,
                                       const DayCounter &dayCounter,
                                       Compounding compounding,
                                       Frequency frequency,
                                       Date settlementDate = Date()) {
        return impl::basisPointValue<Real>(bond, yield, dayCounter, compounding,
                                           frequency, settlementDate);
    }
    ...
    inline static Real yieldValueBasisPoint(const Bond &bond, Rate yield,
                                            const DayCounter &dayCounter,
                                            Compounding compounding,
                                            Frequency frequency,
                                            Date settlementDate = Date()) {
        return impl::yieldValueBasisPoint<Real>(
            bond, yield, dayCounter, compounding, frequency, settlementDate);
    }
    //@}

    //! \name Z-spread functions
    //@{
    inline static Real cleanPrice(
        const Bond &bond, const boost::shared_ptr<YieldTermStructure> &discount,
        Spread zSpread, const DayCounter &dayCounter, Compounding compounding,
        Frequency frequency, Date settlementDate = Date()) {
        return impl::cleanPrice<Real>(bond, discount, zSpread, dayCounter,
                                      compounding, frequency, settlementDate);
    }
    inline static Real yieldTermStructurePV01(
        const Bond &bond, const boost::shared_ptr<YieldTermStructure> &discount,
        const DayCounter &dayCounter, Compounding compounding,
        Frequency frequency, Date settlementDate = Date()) {
        return impl::yieldTermStructurePV01<Real>(
            bond, discount, dayCounter, compounding, frequency, settlementDate);
    }
    inline static Spread
    zSpread(const Bond &bond, Real cleanPrice,
            const boost::shared_ptr<YieldTermStructure> &discount,
            const DayCounter &dayCounter, Compounding compounding,
            Frequency frequency, Date settlementDate = Date(),
            Real accuracy = 1.0e-10, Size maxIterations = 100,
            Rate guess = 0.0) {
        return impl::zSpread<Real>(bond, cleanPrice, discount, dayCounter,
                                   compounding, frequency, settlementDate,
