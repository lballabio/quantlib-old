...
class InterpolatedPiecewiseZeroSpreadedTermStructure
  ...
  public:
    InterpolatedPiecewiseZeroSpreadedTermStructure(
        ...
        const Interpolator<Real> &factory = Interpolator<Real>());
  ...
  private:
    ...
    Frequency freq_;
    DayCounter dc_;
    Interpolator<Real> factory_;
    Interpolation interpolator_;
};

//! Piecewise zero-spreaded yield curve based on linear interpolation of zero
// rates
/*! \ingroup yieldtermstructures */

typedef InterpolatedPiecewiseZeroSpreadedTermStructure<Linear>
    PiecewiseZeroSpreadedTermStructure;

// inline definitions

template <template <class> class T>
inline InterpolatedPiecewiseZeroSpreadedTermStructure<T>::
    InterpolatedPiecewiseZeroSpreadedTermStructure(
        const Handle<YieldTermStructure> &h,
        const std::vector<Handle<Quote> > &spreads,
        const std::vector<Date> &dates, Compounding comp, Frequency freq,
        const DayCounter &dc, const T<Real> &factory)
    : originalCurve_(h), spreads_(spreads), dates_(dates), times_(dates.size()),
      spreadValues_(dates.size()), comp_(comp), freq_(freq), dc_(dc),
      factory_(factory) {
...
template <template <class> class T>
inline DayCounter
InterpolatedPiecewiseZeroSpreadedTermStructure<T>::dayCounter() const {
    return originalCurve_->dayCounter();
}

template <template <class> class T>
inline Calendar
InterpolatedPiecewiseZeroSpreadedTermStructure<T>::calendar() const {
    return originalCurve_->calendar();
}

template <template <class> class T>
inline Natural
InterpolatedPiecewiseZeroSpreadedTermStructure<T>::settlementDays() const {
...
InterpolatedPiecewiseZeroSpreadedTermStructure<T>::zeroYieldImpl(Time t) const {
    ...
    InterestRate spreadedRate(zeroRate + spread, zeroRate.dayCounter(),
...
template <template <class> class T>
inline void
InterpolatedPiecewiseZeroSpreadedTermStructure<T>::updateInterpolation() {
    for (Size i = 0; i < dates_.size(); i++) {
        times_[i] = timeFromReference(dates_[i]);
        spreadValues_[i] = spreads_[i]->value();
    }
    interpolator_ = factory_.interpolate(times_.begin(), times_.end(),
                                         spreadValues_.begin());
}
}
