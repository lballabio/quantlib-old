...
*/
template <class T>
class ZeroSpreadedTermStructure_t : public ZeroYieldStructure_t<T> {
  public:
    ZeroSpreadedTermStructure_t(const Handle<YieldTermStructure_t<T> > &,
  ...
  protected:
    ...
    T zeroYieldImpl(Time) const;
    //! returns the spreaded forward rate
    /* This method must disappear should the spread become a curve */
    T forwardImpl(Time) const;

  private:
    Handle<YieldTermStructure_t<T> > originalCurve_;
...
typedef ZeroSpreadedTermStructure_t<Real> ZeroSpreadedTermStructure;

template <class T>
inline ZeroSpreadedTermStructure_t<T>::ZeroSpreadedTermStructure_t(
    const Handle<YieldTermStructure_t<T> > &h,
    const Handle<Quote_t<T> > &spread, Compounding comp, Frequency freq,
    const DayCounter &dc)
...
template <class T>
inline DayCounter ZeroSpreadedTermStructure_t<T>::dayCounter() const {
    return originalCurve_->dayCounter();
}

template <class T>
inline Calendar ZeroSpreadedTermStructure_t<T>::calendar() const {
    return originalCurve_->calendar();
}

template <class T>
inline Natural ZeroSpreadedTermStructure_t<T>::settlementDays() const {
    return originalCurve_->settlementDays();
}

template <class T>
inline const Date &ZeroSpreadedTermStructure_t<T>::referenceDate() const {
    return originalCurve_->referenceDate();
}

template <class T> inline Date ZeroSpreadedTermStructure_t<T>::maxDate() const {
    return originalCurve_->maxDate();
}

template <class T> inline Time ZeroSpreadedTermStructure_t<T>::maxTime() const {
    return originalCurve_->maxTime();
}

template <class T> inline void ZeroSpreadedTermStructure_t<T>::update() {
...
template <class T>
inline T ZeroSpreadedTermStructure_t<T>::zeroYieldImpl(Time t) const {
...
template <class T>
inline T ZeroSpreadedTermStructure_t<T>::forwardImpl(Time t) const {
