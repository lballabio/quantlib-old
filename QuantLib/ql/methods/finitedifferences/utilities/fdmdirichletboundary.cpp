...
/*
 ...
 Cpoyright (C) 2012 Peter Caspers
...
namespace QuantLib {
	...
	FdmDirichletBoundary::FdmDirichletBoundary(
        ...
        : mesher_(mesher), direction_(direction), side_(side), valueOnBoundary_(0.0),
    ...
    void FdmDirichletBoundary::applyAfterApplying(Array& rhs) const {
		QL_REQUIRE(!spatialDependentBoundary_ || indicies_.size() == valuesOnBoundary_.size(), 
                   "values on boundary size (" << valuesOnBoundary_.size() << 
                   ") does not match hypersurface size (" << indicies_.size() << ")");
		for (std::vector<Size>::const_iterator iter = indicies_.begin();
             iter != indicies_.end(); ++iter) {
            rhs[*iter] = spatialDependentBoundary_ ? valuesOnBoundary_[iter - indicies_.begin()] : 
                valueOnBoundary_;
        }
    }
