...
/*
 ...
 Copyright (C) 2015 Cheng Li, DataYes
...
/*! \ingroup instruments
    ...
    \test calculations are tested by checking results against
          cached values.
*/
template <class T = Real> class FixedRateBond_t : public Bond_t<T> {
  public:
    //! simple annual compounding coupon rates
    FixedRateBond_t(Natural settlementDays, T faceAmount,
                    const Schedule &schedule, const std::vector<T> &coupons,
                    const DayCounter &accrualDayCounter,
                    BusinessDayConvention paymentConvention = Following,
                    T redemption = 100.0, const Date &issueDate = Date(),
    ...
    FixedRateBond_t(Natural settlementDays, const Calendar &couponCalendar,
                    T faceAmount, const Date &startDate,
                    const Date &maturityDate, const Period &tenor,
...
template <class T>
FixedRateBond_t<T>::FixedRateBond_t(
    Natural settlementDays, T faceAmount, const Schedule &schedule,
    const std::vector<T> &coupons, const DayCounter &accrualDayCounter,
    BusinessDayConvention paymentConvention, T redemption,
    const Date &issueDate, const Calendar &paymentCalendar,
    ...
    : Bond_t<T>(settlementDays,
                paymentCalendar == Calendar() ? schedule.calendar()
                                              : paymentCalendar,
                issueDate),
...
template <class T>
FixedRateBond_t<T>::FixedRateBond_t(
    Natural settlementDays, const Calendar &calendar, T faceAmount,
    const Date &startDate, const Date &maturityDate, const Period &tenor,
    const std::vector<T> &coupons, const DayCounter &accrualDayCounter,
    BusinessDayConvention accrualConvention,
    BusinessDayConvention paymentConvention, T redemption,
    const Date &issueDate, const Date &stubDate, DateGeneration::Rule rule,
    bool endOfMonth, const Calendar &paymentCalendar,
    const Period &exCouponPeriod, const Calendar &exCouponCalendar,
    const BusinessDayConvention exCouponConvention, bool exCouponEndOfMonth)
    ...
    Schedule schedule(startDate, this->maturityDate_, tenor, calendar,
                      accrualConvention, accrualConvention, rule, endOfMonth,
...
FixedRateBond_t<T>::FixedRateBond_t(
    ...
    : Bond(settlementDays, paymentCalendar == Calendar() ? schedule.calendar()
                                                         : paymentCalendar,
           issueDate),
