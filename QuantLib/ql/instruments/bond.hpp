...
template <class> class YieldTermStructure_t;
typedef YieldTermStructure_t<Real> YieldTermStructure;

template <class T> class Bond_t;
...
T yield(const Bond_t<T> &bond, T cleanPrice, const DayCounter &dayCounter,
        ...
        Real accuracy = 1.0e-10, Size maxIterations = 100, Rate guess = 0.05);
...
T zSpread(const Bond_t<T> &bond, T cleanPrice,
          ...
          Real accuracy = 1.0e-10, Size maxIterations = 100, Rate guess = 0.0);
...
/*! Derived classes must fill the uninitialized data members.
    ...
    - price/yield calculations are checked against known good
      values.
*/
template <class T> class Bond_t : public Instrument_t<T> {
  public:
    ...
    */
    ...
    Bond_t(Natural settlementDays, const Calendar &calendar,
    ...
    */
    Bond_t(Natural settlementDays, const Calendar &calendar, Real faceAmount,
           const Date &maturityDate, const Date &issueDate = Date(),
    ...
    const typename Leg_t<T>::Type &cashflows() const {
        return this->cashflows_;
    }
    /*! returns just the redemption flows (not interest payments) */
    const typename Leg_t<T>::Type &redemptions() const {
    ...
    */
    ...
    /*! The default bond settlement and theoretical price are used
        for calculation.
    */
    T yield(const DayCounter &dc, Compounding comp, Frequency freq,
            Real accuracy = 1.0e-8, Size maxEvaluations = 100) const;

    //! clean price given a yield and settlement date
    /*! The default bond settlement is used if no date is given. */
    T cleanPrice(T yield, const DayCounter &dc, Compounding comp,
                 Frequency freq, Date settlementDate = Date()) const;

    //! dirty price given a yield and settlement date
    /*! The default bond settlement is used if no date is given. */
    T dirtyPrice(T yield, const DayCounter &dc, Compounding comp,
                 Frequency freq, Date settlementDate = Date()) const;
    ...
    /*! The default bond settlement is used if no date is given. */
    T yield(T cleanPrice, const DayCounter &dc, Compounding comp,
            Frequency freq, Date settlementDate = Date(),
            Real accuracy = 1.0e-8, Size maxEvaluations = 100) const;

    //! Zero spread calculation
    virtual T
    zSpread(T cleanPrice,
            const boost::shared_ptr<YieldTermStructure_t<T> > &yieldCurve,
            const DayCounter &dayCounter, Compounding compounding,
            Frequency frequency, Date settlementDate = Date(),
            Real accuracy = 1.0e-10, Size maxIterations = 100,
            Rate guess = 0.0);
  ...
  protected:
    ...
    */
    void addRedemptionsToCashflows(
        const std::vector<T> &redemptions = std::vector<T>());
    ...
    /*! This method can be called by derived classes in order to
        ...
        data members.
    */
    void setSingleRedemption(T notional, T redemption, const Date &date);
    ...
    */
    void
    setSingleRedemption(T notional,
...
template <class T> class Bond_t<T>::results : public Instrument_t<T>::results {
  public:
    T settlementValue;
    void reset() {
...
template <class T> inline Natural Bond_t<T>::settlementDays() const {
    return settlementDays_;
}

template <class T> inline const Calendar &Bond_t<T>::calendar() const {
    return calendar_;
}

template <class T> inline const std::vector<T> &Bond_t<T>::notionals() const {
    return notionals_;
}

template <class T> inline Date Bond_t<T>::issueDate() const {
    return issueDate_;
}

template <class T>
Bond_t<T>::Bond_t(Natural settlementDays, const Calendar &calendar,
                  const Date &issueDate, const typename Leg_t<T>::Type &coupons)
    : settlementDays_(settlementDays), calendar_(calendar), cashflows_(coupons),
...
template <class T>
Bond_t<T>::Bond_t(Natural settlementDays, const Calendar &calendar,
                  Real faceAmount, const Date &maturityDate,
...
template <class T> bool Bond_t<T>::isExpired() const {
    ...
    return CashFlows::isExpired<T>(cashflows_, true,
                                   Settings::instance().evaluationDate());
}

template <class T> T Bond_t<T>::notional(Date d) const {
    if (d == Date())
        d = settlementDate();
    ...
    std::vector<Date>::const_iterator i = std::lower_bound(
        notionalSchedule_.begin() + 1, notionalSchedule_.end(), d);
...
template <class T> Date Bond_t<T>::startDate() const {
    return impl::startDate<T>(*this);
}

template <class T> Date Bond_t<T>::maturityDate() const {
...
template <class T> bool Bond_t<T>::isTradable(Date d) const {
    return impl::isTradable<T>(*this, d);
}

template <class T> Date Bond_t<T>::settlementDate(Date d) const {
    if (d == Date())
...
template <class T> T Bond_t<T>::cleanPrice() const {

    return dirtyPrice() - accruedAmount(settlementDate());
}

template <class T> T Bond_t<T>::dirtyPrice() const {
...
template <class T> T Bond_t<T>::settlementValue() const {
...
template <class T> T Bond_t<T>::settlementValue(T cleanPrice) const {
    T dirtyPrice = cleanPrice + accruedAmount(settlementDate());
    return dirtyPrice / 100.0 * notional(settlementDate());
}

template <class T>
T Bond_t<T>::yield(const DayCounter &dc, Compounding comp, Frequency freq,
                   Real accuracy, Size maxEvaluations) const {
    T currentNotional = notional(settlementDate());
    if (currentNotional == 0.0)
        return 0.0;

    return impl::yield<T>(*this, cleanPrice(), dc, comp, freq, settlementDate(),
                          accuracy, maxEvaluations);
}

template <class T>
T Bond_t<T>::cleanPrice(T y, const DayCounter &dc, Compounding comp,
                        Frequency freq, Date settlement) const {
    return impl::cleanPrice<T>(*this, y, dc, comp, freq, settlement);
}

template <class T>
T Bond_t<T>::dirtyPrice(T y, const DayCounter &dc, Compounding comp,
                        Frequency freq, Date settlement) const {
...
template <class T>
T Bond_t<T>::yield(T cleanPrice, const DayCounter &dc, Compounding comp,
                   Frequency freq, Date settlement, Real accuracy,
                   Size maxEvaluations) const {
    T currentNotional = notional(settlement);
    if (currentNotional == 0.0)
        return 0.0;

    return impl::yield<T>(*this, cleanPrice, dc, comp, freq, settlement,
                          accuracy, maxEvaluations);
}

template <class T>
T Bond_t<T>::zSpread(
    T cleanPrice, const boost::shared_ptr<YieldTermStructure_t<T> > &yieldCurve,
    const DayCounter &dayCounter, Compounding compounding, Frequency frequency,
    Date settlementDate, Real accuracy, Size maxIterations, Rate guess) {
    return impl::zSpread<T>(*this, cleanPrice, yieldCurve, dayCounter,
                            compounding, frequency, settlementDate, accuracy,
                            maxIterations, guess);
}

template <class T> T Bond_t<T>::accruedAmount(Date settlement) const {
    T currentNotional = notional(settlement);
    if (currentNotional == 0.0)
        return 0.0;
    ...
    return CashFlows::accruedAmount<T>(this->cashflows(), false, settlement) *
           100.0 / notional(settlement);
}

template <class T> T Bond_t<T>::nextCouponRate(Date settlement) const {
    return impl::nextCouponRate<T>(*this, settlement);
}

template <class T> T Bond_t<T>::previousCouponRate(Date settlement) const {
    return impl::previousCouponRate<T>(*this, settlement);
}

template <class T> Date Bond_t<T>::nextCashFlowDate(Date settlement) const {
    return impl::nextCashFlowDate<T>(*this, settlement);
}

template <class T> Date Bond_t<T>::previousCashFlowDate(Date settlement) const {
    return impl::previousCashFlowDate<T>(*this, settlement);
}

template <class T> void Bond_t<T>::setupExpired() const {
...
void Bond_t<T>::setupArguments(PricingEngine::arguments *args) const {
    Bond_t<T>::arguments *arguments =
        dynamic_cast<Bond_t<T>::arguments *>(args);
...
void Bond_t<T>::addRedemptionsToCashflows(const std::vector<T> &redemptions) {
    ...
    for (Size i = 1; i < notionalSchedule_.size(); ++i) {
        T R = i < redemptions.size() ? redemptions[i] : !redemptions.empty()
                                                            ? redemptions.back()
                                                            : 100.0;
        ...
        if (i < this->notionalSchedule_.size() - 1)
            payment.reset(
                new AmortizingPayment_t<T>(amount, notionalSchedule_[i]));
        else
            payment.reset(
                new Redemption_t<T>(amount, this->notionalSchedule_[i]));
...
template <class T>
void Bond_t<T>::setSingleRedemption(T notional, T redemption,
                                    const Date &date) {
...
template <class T>
void Bond_t<T>::setSingleRedemption(
    T notional, const boost::shared_ptr<CashFlow_t<T> > &redemption) {
...
template <class T> void Bond_t<T>::calculateNotionalsFromCashflows() {
...
template <class T> void Bond_t<T>::arguments::validate() const {
