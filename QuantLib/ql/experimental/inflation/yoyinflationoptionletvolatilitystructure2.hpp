...
class InterpolatedYoYOptionletVolatilityCurve
  ...
  public:
    ...
    */
    InterpolatedYoYOptionletVolatilityCurve(
        Natural settlementDays, const Calendar &, BusinessDayConvention bdc,
        const DayCounter &dc, const Period &lag, Frequency frequency,
        bool indexIsInterpolated, const std::vector<Date> &d,
        const std::vector<Volatility> &v, Rate minStrike, Rate maxStrike,
  ...
  protected:
    ...
    InterpolatedYoYOptionletVolatilityCurve(
        Natural settlementDays, const Calendar &, BusinessDayConvention bdc,
        const DayCounter &dc, const Period &lag, Frequency frequency,
        bool indexIsInterpolated, Rate minStrike, Rate maxStrike,
    ...
    Interpolator1D<Real> interpolator_;
...
InterpolatedYoYOptionletVolatilityCurve<Interpolator1D>::
    ...
    : YoYOptionletVolatilitySurface(settlementDays, cal, bdc, dc, lag,
      ...
      dates_(d), data_(v), interpolator_(interpolator), minStrike_(minStrike),
      maxStrike_(maxStrike) {
    QL_REQUIRE(d.size() == v.size(), "must have same number of dates and vols: "
                                         << d.size() << " vs " << v.size());
    ...
    interpolation_ =
        interpolator_.interpolate(times_.begin(), times_.end(), data_.begin());
...
InterpolatedYoYOptionletVolatilityCurve<Interpolator1D>::
    InterpolatedYoYOptionletVolatilityCurve(
        ...
        const DayCounter &dc, const Period &lag, Frequency frequency,
        bool indexIsInterpolated, Rate minStrike, Rate maxStrike,
        Volatility baseYoYVolatility, const Interpolator1D<Real> &interpolator)
    : YoYOptionletVolatilitySurface(settlementDays, cal, bdc, dc, lag,
                                    frequency, indexIsInterpolated),
      interpolator_(interpolator), minStrike_(minStrike),
      maxStrike_(maxStrike) {
...
template <template <class> class Interpolator1D>
inline Volatility
InterpolatedYoYOptionletVolatilityCurve<Interpolator1D>::volatilityImpl(
    const Time t, Rate) const {
