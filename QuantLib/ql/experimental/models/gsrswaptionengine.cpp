...
namespace QuantLib {
    ...
    void GsrSwaptionEngine::calculate() const {
        ...
        Size N=(int)(yN*T+0.5);
        std::vector<Real> t(N);
        std::vector<Real> yF(N,0.0),yD(N,0.0);
        for(Size i=0;i<N;++i) {
            t[i]=i*(T/((Real)N-1));
        }
                ...
                if(expiry0 >today) {
                    ...
                    for(Size l=k1;l<arguments_.floatingCoupons.size();l++) {
                        floatingLegNpv += arguments_.nominal * arguments_.floatingAccrualTimes[l] *
                                (arguments_.floatingSpreads[l]+model_->forwardRate(
                                arguments_.floatingFixingDates[l],arguments_.swap->iborIndex(),expiry0,z[k],spreadF_)) * 
                    ...
                    for(Size l=j1;l<arguments_.fixedCoupons.size();l++) {
                        fixedLegNpv += arguments_.fixedCoupons[l] * 
                            model_->zerobond(arguments_.fixedPayDates[l],expiry0,z[k],spreadD_);
                    }
                    npv0[k] = std::max( npv0[k], (type==Option::Call ? 1.0 : -1.0) * ( floatingLegNpv - fixedLegNpv ) / 
