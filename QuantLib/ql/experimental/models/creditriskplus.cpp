...
namespace QuantLib {
    ...
    CreditRiskPlus::CreditRiskPlus(const std::vector<Real>& exposure,
                                   const std::vector<Real>& pd,
                                   const std::vector<Size>& sector,
                                   const std::vector<Real>& relativeDefaultVariance,
                                   const Matrix& correlation,
                                   const Real unit) :
        exposure_(exposure), pd_(pd), sector_(sector), relativeDefaultVariance_(relativeDefaultVariance),
        correlation_(correlation), unit_(unit) {

        m_ = exposure_.size();
    
        QL_REQUIRE(m_ > 0, "no exposures given");
        QL_REQUIRE(m_ == pd_.size(), "number of exposures (" << m_ << 
                   ") must be equal to number of pds (" << pd_.size() << ")");
        QL_REQUIRE(m_ == sector_.size(), "number of exposures (" << m_ << 
                   ") must be equal to number of sectors (" << sector_.size() << ")");
                   
        n_ = correlation_.rows();
        QL_REQUIRE(correlation_.columns() == n_,"correlation matrix (" << n_ << "," << 
                   correlation_.columns() << ") must be a square matrix");

        el_ = 0.0;
        for(Size i=0;i<m_;i++) {
            QL_REQUIRE(exposure_[i] >= 0.0, "exposure #" << i << " is negative (" << exposure_[i] << ")");
            QL_REQUIRE(pd_[i] > 0.0, "pd #" << i << " is negative (" << pd_[i] << ")");
            QL_REQUIRE(sector_[i] < n_, "sector #" << i << " (" << sector_[i] << ") is out of range 0..." << (n_-1));
    ...
    void CreditRiskPlus::compute() {
        ...
        std::map<unsigned long,Real,std::less<unsigned long>> epsNuC_; // map of nuC_ to expected loss
        ...
        for(Size i=0;i<n_;i++) {
            ...
            for(Size j=0;j<n_;j++) {
                if(j!=i) {
                    sectorSpecTerms_[i] += correlation_[i][j]*
                        std::sqrt(relativeDefaultVariance_[i]*relativeDefaultVariance_[j])*
                        sectorEl_[j];
                }
            }
        }
        ...
        ul_=0.0;
        for(Size i=0;i<n_;i++) {
            sectorUl_ [i] = relativeDefaultVariance_[i]*
                sectorEl_[i]*sectorEl_[i];
            ul_ += sectorUl_[i];
            for(Size j=0;j<n_;j++) {
                if(j!=i) {
        ...
        Real res;
        for(unsigned long n=0;n<upperIndex_-1;n++) { // compute A(n+1) recursively
            res=0.0;
            for(unsigned long j=0;j<=std::min<unsigned long>(maxNu_-1,n);j++) {
                iter=epsNuC_.find(j+1);
                if(iter!=epsNuC_.end()) {
                    res+=(*iter).second*loss_[n-j]*alphaC_;
                    if(j<=n-1) res+=(*iter).second/((Real)(j+1))*((Real)(n-j))*loss_[n-j];
                }
            }
