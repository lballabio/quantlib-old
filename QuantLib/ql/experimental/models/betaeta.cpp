...
const Real BetaEta::p(const Time t0, const Real x0, const Real t,
    ...
    if (eta_ < 0.5) {
        ...
        if (x > -1.0 / beta_) {
            return 0.5 * std::pow(y0 / y, nu) * y0 / (tau - tau0) *
                   (modifiedBesselFunction_i(-nu, y0 * y / (tau - tau0)) +
                    modifiedBesselFunction_i(nu, y0 * y / (tau - tau0))) *
                   exp(-(y * y + y0 * y0) / (2.0 * (tau - tau0))) * dydx(y);
        } else {
            return std::sin(M_PI * nu) * M_1_PI * std::pow(y0 / y, nu) * y /
                   (tau - tau0) *
                   modifiedBesselFunction_k(nu, y0 * y / (tau - tau0)) *
                   exp(-(y * y + y0 * y0) / (2.0 * (tau - tau0))) * dydx(y);
        }
    } else {
        if (close(eta_, 1.0)) {
            // check this, should we return zero here ?
            if (close(x, -1.0 / beta_) || close(x0, -1.0 / beta_))
                return 0.0;
