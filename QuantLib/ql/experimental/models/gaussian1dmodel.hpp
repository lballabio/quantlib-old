...
namespace QuantLib {
    ...
    /*! One factor interest rate model interface class
        ...
        \warning the variance of the state process conditional on $x(t)=x$ must be
    ...
    class Gaussian1dModel : public TermStructureConsistentModel, 
      ...
      public:
        ...
        const Real swapRate(const Date& fixing, const Period& tenor, const Date& referenceDate = Null<Date>(), 
                            const Real y=0.0, boost::shared_ptr<SwapIndex> swapIdx = boost::shared_ptr<SwapIndex>()) const;
