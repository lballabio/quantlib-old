...
CmsSpreadCoupon::CmsSpreadCoupon(const Date &paymentDate, Real nominal,
                                 const Date &startDate, const Date &endDate,
                                 Natural fixingDays,
                                 const boost::shared_ptr<SwapIndex> &index1,
                                 const boost::shared_ptr<SwapIndex> &index2,
                                 Real gearing1, Real gearing2, Spread spread,
                                 const Date &refPeriodStart,
                                 const Date &refPeriodEnd,
                                 const DayCounter &dayCounter, bool isInArrears)
    : FloatingRateCoupon(paymentDate, nominal, startDate, endDate, fixingDays,
                         boost::assign::list_of(index1)(index2),
                         boost::assign::list_of(gearing1)(gearing2)
                             .convert_to_container<std::vector<Real>>(),
                         spread, refPeriodStart, refPeriodEnd, dayCounter,
                         isInArrears) {}
...
CmsSpreadLeg::CmsSpreadLeg(const Schedule &schedule,
    ...
    : schedule_(schedule), index1_(index1), index2_(index2),
      gearing1_(gearing1), gearing2_(gearing2), paymentAdjustment_(Following),
...
CmsSpreadLeg::operator Leg() const {
    return FloatingLeg(
        CmsSpreadCouponFactory(index1_, index2_, gearing1_, gearing2_),
        schedule_, notionals_, paymentDayCounter_, paymentAdjustment_,
        fixingDays_, gearings_, spreads_, caps_, floors_, inArrears_,
        zeroPayments_);
}
}
