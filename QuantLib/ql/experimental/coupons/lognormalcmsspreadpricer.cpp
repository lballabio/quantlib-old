...
const Real LognormalCmsSpreadPricer::integrand(const Real x) const {
    ...
    Real v = M_SQRT2 * x;
    Real h = k_ -
             b_ * s2_ * std::exp((m2_ - 0.5 * v2_ * v2_) * fixingTime_ +
    ...
    phi2 = cnd_->operator()(phi_ * (std::log(a_ * s1_ / h) +
...
void LognormalCmsSpreadPricer::initialize(const FloatingRateCoupon &coupon) {
    ...
    index1_ = coupon_->swapIndex1();
    index2_ = coupon_->swapIndex2();
    gearing1_ = coupon_->gearing1();
    gearing2_ = coupon_->gearing2();
    ...
    if (couponDiscountCurve_.empty())
        couponDiscountCurve_ = index1_->exogenousDiscount()
                                   ? index1_->discountingTermStructure()
                                   : index1_->forwardingTermStructure();
    ...
    c1_ = boost::shared_ptr<CmsCoupon>(new CmsCoupon(
        ...
        coupon_->accrualEndDate(), coupon_->fixingDays(), index1_, 1.0, 0.0,
    ...
    c2_ = boost::shared_ptr<CmsCoupon>(new CmsCoupon(
        coupon_->date(), coupon_->nominal(), coupon_->accrualStartDate(),
        coupon_->accrualEndDate(), coupon_->fixingDays(), index2_, 1.0, 0.0,
        coupon_->referencePeriodStart(), coupon_->referencePeriodEnd(),
        coupon_->dayCounter(), coupon_->isInArrears()));
    ...
    if (fixingDate_ > today_) {
        ...
        name << gearing1_ << "_" << index1_->name() << "_" << gearing2_ << "_"
             << index2_->name();
        std::pair<std::string, Date> key =
            std::make_pair(name.str(), fixingDate_);
        ...
        vol1_ = cmsPricer_->swaptionVolatility()->volatility(
            fixingDate_, index1_->tenor(), swapRate1_);
        vol2_ = cmsPricer_->swaptionVolatility()->volatility(
            fixingDate_, index2_->tenor(), swapRate2_);
...
Real LognormalCmsSpreadPricer::optionletPrice(Option::Type optionType,
    ...
    if (strike >= 0.0) {
        ...
        v2_ = vol1_;
        k_ = -strike;
        res += phi_ * (gearing1_ * adjustedRate1_ + gearing2_ * adjustedRate2_ -
                       strike);
    }

    res += integrator_->operator()(
        std::bind1st(std::mem_fun(&LognormalCmsSpreadPricer::integrand), this));
...
Real LognormalCmsSpreadPricer::capletPrice(Rate effectiveCap) const {
    ...
    if (fixingDate_ <= today_) {
        ...
        const Rate Rs = std::max(coupon_->indexFixing() - effectiveCap, 0.);
        Rate price = Rs * (coupon_->accrualPeriod() *
                           couponDiscountCurve_->discount(paymentDate_));
        return price;
    } else {
...
Real LognormalCmsSpreadPricer::floorletPrice(Rate effectiveFloor) const {
    ...
    if (fixingDate_ <= today_) {
        // the fixing is determined
        const Rate Rs = std::max(effectiveFloor - coupon_->indexFixing(), 0.);
        Rate price = Rs * (coupon_->accrualPeriod() *
                           couponDiscountCurve_->discount(paymentDate_));
        return price;
    } else {
        Real floorletPrice = optionletPrice(Option::Put, effectiveFloor);
        return floorletPrice;
    }
}
