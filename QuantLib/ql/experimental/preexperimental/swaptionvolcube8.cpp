...
namespace QuantLib {
    ...
    SwaptionVolCube8::SwaptionVolCube8(
				...
				const std::vector<Real>& interpolStrikeSpreads)
    : SwaptionVolatilityCube(atmVolStructure, optionTenors, swapTenors,
	  ...
	  interpolStrikeSpreads_(interpolStrikeSpreads.size() > 0 ? interpolStrikeSpreads : strikeSpreads)
    {
    ...
    void SwaptionVolCube8::registerWithParametersGuess()
    {
        for (Size i=0; i<5; i++)
    ...
    void SwaptionVolCube8::performCalculations() const{
    ...
    SwaptionVolCube8::Cube
    SwaptionVolCube8::sabrCalibration(const Cube& marketVolCube) const {
		...
		sabrParametersCube.setLayer(8, slopes);

        return sabrParametersCube;

    }
    void SwaptionVolCube8::sabrCalibrationSection(
    ...
    void SwaptionVolCube8::fillVolatilityCube() const {
    ...
    void SwaptionVolCube8::createSparseSmiles() const {
    ...
    std::vector<Real> SwaptionVolCube8::spreadVolInterpolation(
    ...
    boost::shared_ptr<SmileSection> SwaptionVolCube8::smileSection(Time optionTime, Time swapLength,
    ...
    SwaptionVolCube8::smileSection(const Date& optionDate,
        ...
        const std::vector<Real> sabrParameters = sabrParametersCube(optionTime, swapLen);

        boost::shared_ptr<SmileSection> sabr(new SabrSmileSection(optionTime, sabrParameters[4], sabrParameters));
		boost::shared_ptr<SmileSection> mx(new MxCmsReplSmileSection(sabr,strikeSpreads_,sabrParameters[8]));
    ...
    boost::shared_ptr<SmileSection>
    SwaptionVolCube8::smileSectionImpl(Time optionTime,
	...
	boost::shared_ptr<SmileSection>
    SwaptionVolCube8::smileSectionImpl(const Date& optionDate,
									   const Period& swapTenor) const {
        if (isAtmCalibrated_)
    ...
    Matrix SwaptionVolCube8::sparseSabrParameters() const {
        calculate();
        return sparseParameters_.browse();
    }

    Matrix SwaptionVolCube8::denseSabrParameters() const {
        calculate();
        return denseParameters_.browse();
    }

    Matrix SwaptionVolCube8::marketVolCube() const {
        calculate();
        return marketVolCube_.browse();
    }
    Matrix SwaptionVolCube8::volCubeAtmCalibrated() const {
        calculate();
        return volCubeAtmCalibrated_.browse();
    }

    void SwaptionVolCube8::recalibration(const std::vector<Real>& slope,
                           const Period& swapTenor) {

       QL_REQUIRE(slope.size()==nOptionTenors_,"Slope vector (" << slope.size() << ") must have equal length as number of option tenors (" << nOptionTenors_ << ")");
		...
		for(int i=0;i<nOptionTenors_;i++) {
			parametersGuess_.setElement(1,i,k,slope[i]);
		}
    ...
    SwaptionVolCube8::Cube::Cube(const Cube& o) {
    ...
    SwaptionVolCube8::Cube&
    SwaptionVolCube8::Cube::operator=(const Cube& o) {
        optionTimes_ = o.optionTimes_;
        swapLengths_ = o.swapLengths_;
        optionDates_ = o.optionDates_;
        swapTenors_ = o.swapTenors_;
    ...
    void SwaptionVolCube8::Cube::setElement(Size IndexOfLayer,
                                                        Size IndexOfRow,
                                                        Size IndexOfColumn,
                                                        Real x) {
    ...
    void SwaptionVolCube8::Cube::setPoints(
    ...
    void SwaptionVolCube8::Cube::setLayer(Size i,
                                                      const Matrix& x) {
    ...
    void SwaptionVolCube8::Cube::setPoint(
    ...
    void SwaptionVolCube8::Cube::expandLayers(
    ...
    SwaptionVolCube8::Cube::points() const {
        return points_;
    }

    std::vector<Real> SwaptionVolCube8::Cube::operator()(
    ...
    SwaptionVolCube8::Cube::optionTimes() const {
        return optionTimes_;
    }

    const std::vector<Time>&
    SwaptionVolCube8::Cube::swapLengths() const {
        return swapLengths_;
    }

    void SwaptionVolCube8::Cube::updateInterpolators() const {
        for (Size k=0; k<nLayers_; ++k) {
    ...
    Matrix SwaptionVolCube8::Cube::browse() const {
