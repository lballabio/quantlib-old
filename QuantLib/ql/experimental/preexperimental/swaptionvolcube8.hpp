...
/*! \file swaptionvolcube8.hpp
    \brief Swaption volatility cube, mimiking Murex 3.1.22 CMS Replication Model Smile Pretreatment
*/

// TODO provide different default values for max error, rms error and vega weighted / non weighted calibration
// TODO the same in sabrinterpolation.hpp

#ifndef quantlib_swaption_volcube_8_h
#define quantlib_swaption_volcube_8_h
...
namespace QuantLib {
    ...
    class SwaptionVolCube8 : public SwaptionVolatilityCube {
        class Cube {
          public:
            Cube() {}
      ...
      public:
        SwaptionVolCube8(
			...
			const std::vector<Real>& interpolStrikeSpreads = std::vector<Real>(0));
        ...
        void sabrCalibrationSection(const Cube& marketVolCube,
                                    Cube& parametersCube,
                                    const Period& swapTenor) const;
        void recalibration(const std::vector<Real>& slope,
                           const Period& swapTenor);
     protected:
      ...
      private:
		...
		const std::vector<Real> interpolStrikeSpreads_;
    };
