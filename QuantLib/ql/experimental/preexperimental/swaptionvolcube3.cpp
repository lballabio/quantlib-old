...
namespace QuantLib {
    ...
    void SwaptionVolCube3::performCalculations() const{
        ...
        for (Size i=0; i<nStrikes_; i++) {
			volSpreadsInterpolator_[i] = FlatExtrapolator2D(boost::shared_ptr<Interpolation2D>(
    ...
    SwaptionVolCube3::smileSectionImpl(const Date& optionDate,
        ...
        return boost::shared_ptr<SmileSection>(new
            InterpolatedSmileSection<Moneyness>(optionTime,
                                             strikes,
                                             stdDevs,
                                             atmForward));
    }
