...
namespace QuantLib {
	...
	HW2FAdj::HW2FAdj(const double a, const double b, 
				const std::vector<double>& rho,
				const std::vector<double>& times,
				const std::vector<double>& sigma,
				const std::vector<double>& eta,
				boost::shared_ptr<YieldTermStructure> yieldTS,
				const Size hermitePointsInner,
				const Size hermitePointsOuter,
				const Size mcpaths,
				const Size mcseed) :
	a_(a), b_(b), rho_(rho), times_(times), sigma_(sigma), eta_(eta), yieldTS_(yieldTS),
		mcpaths_(mcpaths),mcseed_(mcseed) {
			innerInt_=new GaussHermiteIntegration(hermitePointsInner);
			outerInt_=new GaussHermiteIntegration(hermitePointsOuter);
	}


	double HW2FAdj::expectationX(const Size index, const Size payIndex) {
		double sum=0.0;
