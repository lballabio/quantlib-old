...
/*! \file bdksmilesection.hpp
    \brief Arbitrage free smile section using the method described in the benim kainth dodgson paper
*/
...
namespace QuantLib {
    ...
    class BdkSmileSection : public SmileSection {
	  ...
	  public:
        ...
        BdkSmileSection(const boost::shared_ptr<SmileSection> source, const Real mu=3.0, const Real nu=3.0, const Real minLeftCutoffStrike=0.0, const Real maxRightCutoffStrike=QL_MAX_REAL, const std::vector<Real>& moneynessGrid = std::vector<Real>(), const Real atm = Null<Real>());
