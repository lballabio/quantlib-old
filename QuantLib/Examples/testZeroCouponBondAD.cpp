...
int main() {
	...
	typedef ZeroCouponBond_t<CppAD::AD<double> > ZeroCouponBondAD;
	...
	for (Size ii = 0; ii < maximumMaturity.size(); ++ii) {
		for (Size jj = 0; jj < portfolioSize.size(); ++jj) {
			...
			boost::shared_ptr<DiscountingBondEngineAD>
				bondEngineAD(new DiscountingBondEngineAD(curveHandleAD));
			boost::shared_ptr<DiscountingBondEngine>
				bondEngine(new DiscountingBondEngine(curveHandle));

			std::vector<boost::shared_ptr<ZeroCouponBond> > portfolio;
			std::vector<boost::shared_ptr<ZeroCouponBondAD> > portfolioAD;
			...
			for (Size j = 0; j < portfolioSize[jj]; ++j) {
				...
				auto bond = boost::make_shared<ZeroCouponBond>(1, NullCalendar(), 100.0, termination);
				auto bondAD = boost::make_shared<ZeroCouponBondAD>(1, NullCalendar(), 100.0, termination);
				bond->setPricingEngine(bondEngine);
				bondAD->setPricingEngine(bondEngineAD);
