...
int main() {
    ...
    while (x <= 0.02) {
        std::cout << x << " "
                  << model.swapRate(startDate, startDate, index, 10 * Years, x,
                                    y)
                  << " "
                  << -std::log(
                         model.zerobond(TARGET().advance(startDate, 10 * Years),
                                        startDate, x, y)) /
                         10.0
                  << std::endl;
        x += 0.0001;
    ...
    std::cout << "dS/dx ( -0.02 ) = "
              ...
              << (model.swapRate(startDate, startDate, index, 10 * Years,
                                 -0.02 + 0.0001, y) -
                  model.swapRate(startDate, startDate, index, 10 * Years, -0.02,
                                 y)) /
                     0.0001
              << std::endl;
    std::cout << "dS/dx ( 0.00 ) = "
              ...
              << (model.swapRate(startDate, startDate, index, 10 * Years,
                                 0.0 + 0.0001, y) -
                  model.swapRate(startDate, startDate, index, 10 * Years, 0.0,
                                 y)) /
                     0.0001
              << std::endl;
    std::cout << "dS/dx ( +0.02 ) = "
