	...
	for (Size ii = 0; ii < maximumMaturity.size(); ++ii) {
		for (Size jj = 0; jj < portfolioSize.size(); ++jj) {
			...
			for (Size j = 0; j < portfolioSize[jj]; ++j) {
				...
				auto bond = boost::make_shared<FixedRateBond>(1, 100.0, fixedSchedule, std::vector<Real>(1, couponRate), Actual360());
				auto bondAD = boost::make_shared<FixedRateBondAD>(1, 100.0, fixedSchedule, std::vector<CppAD::AD<double> >(1, CppAD::AD<double>(couponRate)), Actual360());
