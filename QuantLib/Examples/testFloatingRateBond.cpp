...
int main() {
	...
	typedef FloatingRateBond_t<CppAD::AD<double> > FloatingRateBondAD;
	...
	for (Size ii = 0; ii < maximumMaturity.size(); ++ii) {
		for (Size jj = 0; jj < portfolioSize.size(); ++jj) {
			...
			boost::shared_ptr<DiscountingBondEngineAD>
				bondEngneAD(new DiscountingBondEngineAD(curveHandleAD));
			boost::shared_ptr<DiscountingBondEngine>
				bondEngne(new DiscountingBondEngine(curveHandle));

			std::vector<boost::shared_ptr<FloatingRateBond> > portfolio;
			std::vector<boost::shared_ptr<FloatingRateBondAD> > portfolioAD;
			...
			for (Size j = 0; j < portfolioSize[jj]; ++j) {
				...
				auto bond = boost::make_shared<FloatingRateBond>(1, 100.0, floatSchedule, euribor6mYts, Actual360());
				auto bondAD = boost::make_shared<FloatingRateBondAD>(1, 100.0, floatSchedule, euribor6mYtsAD, Actual360());
