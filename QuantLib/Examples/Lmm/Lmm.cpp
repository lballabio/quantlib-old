...
class A {
public:
    A(const Handle<B>& b=Handle<B>()) : b_(b) {}
    //A(const boost::shared_ptr<YieldTermStructure>& b= boost::shared_ptr<YieldTermStructure>()) : b_(b) {}
    //A(const Date& b=Null<Date>()) : b_(b) {}
    //A(const int& b=5) : b_(b) {}
    const bool empty() { return b_.empty(); }
    //const bool empty() { return b_ == NULL; }
    //const bool empty() { return b_ == Null<Date>(); }
    //const int b() { return b_; }
private:
    const Handle<B>& b_;
    //const boost::shared_ptr<YieldTermStructure>& b_;
    //const Date& b_;
    //const int& b_;
};
...
//     std::cout << "Libor Market Model Demo --- CMS Swaption Pricing" << std::endl;
...
// 	for(int i=0;i<noDep;i++) irratehelpers.push_back(boost::shared_ptr<RateHelper>(new DepositRateHelper(
//         Handle<Quote>(irquotes[i]),tenors[i],2,TARGET(),ModifiedFollowing,false,Actual360()))); 
//     // check: fixing days = 2 for all tenors ?
// 	for(int i=noDep;i<noDep+noSwap;i++) irratehelpers.push_back(boost::shared_ptr<RateHelper>(new SwapRateHelper(
//         Handle<Quote>(irquotes[i]),tenors[i],TARGET(), Annual, ModifiedFollowing, Thirty360(), euribor6mBootstrap)));

// 	boost::shared_ptr<YieldTermStructure> yts(new PiecewiseYieldCurve<ZeroYield,Linear>(0,TARGET(),irratehelpers,
//                                                             Actual365Fixed())); // discount is 1.0 on TODAY (evalDate)
...
// 	Real smileValAbs[] = { 0.2725, 0.2161, 0.00, 0.1706, 0.1741, // opt1swp1 3m20y // this is NOT the spread, 
...
// 	Size noSwSmile=3, noOpSmile=8, noSmileSpreads=5; // put number of swap tenors, option tenors and strike 
//                                                      // spreads for smile here (BBG)
...
// 			const Size swpEndIdx[] = { 52,52,52,52,52, 52,52,52,52,52, 52,52,52,52,52, 52,52,52,52,52, 52,52,52,52,52, 
//                                        52,52,52,52,52, 52,52,52,52,52 };
// 			const Real atmOffset[] = { -0.02, -0.01, 0.0, 0.01, 0.02, -0.02, -0.01, 0.0, 0.01, 0.02, 
...
// 				marketPrices_.push_back(blackFormula(Option::Call,strikes_[i],forwards_[i],
...
// 			corr_ = boost::shared_ptr<PiecewiseConstantCorrelation>(new TimeHomogeneousForwardCorrelation(
//                                                                                   corrMatrix,evolution_.rateTimes()));
...
// 		Disposable<Array> operator()(const std::vector<Real>& displacements, const std::vector<Real>& volatilities, 
//                                      const Real a, const Real b, const Real c, const Real d,
//                                      const Real volvol, const Real meanRev, const Size paths=CALIBRATIONPATHS) const { 
//             // put default paths here (used in calibration)

// 			boost::shared_ptr<MarketModel> model(new AbcdVol(a,b,c,d,volatilities,corr_,evolution_,FACTORS,
//                                                              todaysForwards_,displacements)); // factors = 10
// 			long seed = SEED;
// 			MTBrownianGeneratorFactory generatorFactory(seed); // what rng do we use ? MT or SOBOL ?
...
//             if(close(volvol,0.0)) { // the criterion whether to use a shifted bgm or a stoch vol bgm is the volvol
//                 // SBGM
//                 evolver = boost::shared_ptr<MarketModelEvolver>(new LogNormalFwdRatePc(model, generatorFactory,numeraires_,0));
//                 // what does initial step mean?
//                 // boost::shared_ptr<MarketModelEvolver> evolver(new LogNormalFwdRateEuler(model, generatorFactory,
//                 // numeraires_,0)); 
//                 // what does initial step mean?
//             }
//             else {
//                 // FLSV
//                 boost::shared_ptr<MarketModelVolProcess> volProcess(new SquareRootAndersen(1.0,meanRev,volvol,1.0,
//                                                      evolution_.evolutionTimes(),8,0.5,0.5,1.5)); // fancy parameters here ?
//                 evolver = boost::shared_ptr<MarketModelEvolver>(new SVDDFwdRatePc(model, generatorFactory, volProcess, 
//                                                                                                        2, 2, numeraires_));
//             }
...
// 			boost::shared_ptr<SequenceStatisticsInc> stats(new SequenceStatisticsInc(
//                                                                              calibrationInstruments_.numberOfProducts()));
// 			engine.multiplePathValues(*stats, paths);

// 			Array result(calibrationInstruments_.size());
// 			std::cout << "#\tfixingT\tstrike \tpricemk\tpricemd\tvolmk  \tvolmd  \tweight // " << std::endl;
...
// 	// 			try { modelv = blackFormulaImpliedStdDev(Option::Call,strikes_[i],forwards_[i],modelp,annuities_[i]) / 
//     //                     sqrt(fixingTimes_[i]);
// 	// 			} catch(...) {}
...
// 	// 			std::cout << i << "\t" << fixingTimes_[i] << "\t" << strikes_[i] << "\t" << marketp << "\t" << modelp 
//     //                       << "\t" << marketv << "\t" << modelv << "\t" << weights_[i] << std::endl;
// 	// 		}
...
//     // // displacements are capped at 100%
//     // // volatilities are capped at 2.0
//     // // if volvol', meanRev' is not given, a shifted BGM is calibrated, otherwise a SV BGM 
//     // // (zero volvol and meanrev is feeded to the calibration error class)
...
// 	// 		Real d0,dinf,dl,v0,vinf,vl,volvol=0.0,meanrev=0.0;
// 	// 		d0=x[4]*x[4];
// 	// 		dinf=x[5]*x[5];
// 	// 		dl=x[6]*x[6];
// 	// 		v0=x[7]*x[7];
...
// 	// 	Array start(4+3+3/*+2*/); // 4 abcd, 3 displacement, 3 voladjusterrs, 2 volvol,meanrev (ONLY IF STOCH VOL SHOULD BE USED!)
...
// 	// 	LevenbergMarquardt opt; // OPTIMIZER FOR CALIBRATION
// 	// 	//Simplex opt(0.01);
...
//     //     Real volvol=0.0, meanrev=0.0;
//     //     if(result.size()==12) {
//     //         volvol=result[10]*result[10]; 
//     //         meanrev=result[11]*result[11];
//     //     }
