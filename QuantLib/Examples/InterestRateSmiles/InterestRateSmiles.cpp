...
int main(int, char * []) {
    ...
    Real strike = 0.0010; // we start at 10bp ...

    while (strike <= 1.00) { // ... and go to 100% ...

        // debug: localvol function for sabr hardcoded
        Real ytmp = 1.0/(sabrParams[0]*(1.0-sabrParams[1])) * (pow(forward,1.0-sabrParams[1])-pow(strike,1.0-sabrParams[1]));
        Real Jtmp = sqrt(1.0+sabrParams[2]*sabrParams[2]*ytmp*ytmp-2.0*sabrParams[3]*sabrParams[2]*ytmp);
        Real localVol = Jtmp * pow(strike,sabrParams[1]) * sabrParams[0];
        // end debug
