            ...
            for (Size k = 0; k < LENGTH(coupons); k++) {
                for (Size l = 0; l < LENGTH(frequencies); l++) {
                    for (Size n = 0; n < LENGTH(compounding); n++) {
                        ...
                        Date dated = vars.calendar.advance(
                            vars.today, issueMonths[i], Months);
                        Date issue = dated;
                        Date maturity =
                            vars.calendar.advance(issue, lengths[j], Years);

                        Schedule sch(dated, maturity, Period(frequencies[l]),
                                     vars.calendar, accrualConvention,
                                     accrualConvention,
                        ...
                        for (Size m = 0; m < LENGTH(yields); m++) {
                            ...
                            Real price = BondFunctions::cleanPrice(
                                bond, yields[m], bondDayCount, compounding[n],
                                frequencies[l]);
                            Rate calculated = BondFunctions::yield(
                                bond, price, bondDayCount, compounding[n],
                                frequencies[l], Date(), tolerance,
                                maxEvaluations);

                            if (std::fabs(yields[m] - calculated) > tolerance) {
                                // the difference might not matter
                                Real price2 = BondFunctions::cleanPrice(
                                    bond, calculated, bondDayCount,
                                    compounding[n], frequencies[l]);
                                if (std::fabs(price - price2) / price >
                                    tolerance) {
                                    BOOST_FAIL(
        ...
        for (Size j = 0; j < LENGTH(lengths); j++) {
            for (Size k = 0; k < LENGTH(coupons); k++) {
                for (Size l = 0; l < LENGTH(frequencies); l++) {
                    ...
                    Schedule sch(dated, maturity, Period(frequencies[l]),
                                 vars.calendar, accrualConvention,
                                 accrualConvention, DateGeneration::Backward,
                                 false);
                    ...
                    Rate calculated = BondFunctions::atmRate(
                        bond, **disc, bond.settlementDate(), price);
                ...
                for (Size l = 0; l < LENGTH(frequencies); l++) {
                    for (Size n = 0; n < LENGTH(compounding); n++) {
                        ...
                        for (Size m = 0; m < LENGTH(spreads); m++) {
                            ...
                            Real price = BondFunctions::cleanPrice(
                                bond, *discountCurve, spreads[m], bondDayCount,
                                compounding[n], frequencies[l]);
                            Spread calculated = BondFunctions::zSpread(
                                bond, price, *discountCurve, bondDayCount,
                                compounding[n], frequencies[l], Date(),
                                tolerance, maxEvaluations);

                            if (std::fabs(spreads[m] - calculated) >
                                tolerance) {
                                // the difference might not matter
                                Real price2 = BondFunctions::cleanPrice(
                                    bond, *discountCurve, calculated,
    ...
    for (Size j = 0; j < LENGTH(lengths); j++) {
        for (Size k = 0; k < LENGTH(coupons); k++) {
            for (Size l = 0; l < LENGTH(frequencies); l++) {
                ...
                FixedRateBond bond(settlementDays, vars.faceAmount, sch,
                                   std::vector<Rate>(1, coupons[k]),
                                   bondDayCount, paymentConvention, redemption,
                                   issue);
                ...
                for (Size m = 0; m < LENGTH(yields); m++) {
                    ...
                    Real price =
                        BondFunctions::cleanPrice(bond, yields[m], bondDayCount,
                                                  Continuous, frequencies[l]);
                    ...
                    Rate calculatedYield = BondFunctions::yield(
                        bond, calculatedPrice, bondDayCount, Continuous,
                        frequencies[l], bond.settlementDate(), tolerance,
                        maxEvaluations);
...
void BondTest::testCached() {
    ...
    Schedule sch1(Date(31, October, 2004), Date(31, October, 2006),
                  Period(freq), bondCalendar, Unadjusted, Unadjusted,
                  DateGeneration::Backward, false);

    FixedRateBond bond1(settlementDays, vars.faceAmount, sch1,
                        std::vector<Rate>(1, 0.025), bondDayCount,
    ...
    Schedule sch2(Date(15, November, 2004), Date(15, November, 2009),
                  Period(freq), bondCalendar, Unadjusted, Unadjusted,
                  DateGeneration::Backward, false);

    FixedRateBond bond2(settlementDays, vars.faceAmount, sch2,
                        std::vector<Rate>(1, 0.035), bondDayCount,
    ...
    price = BondFunctions::cleanPrice(bond1, marketYield1, bondDayCount,
                                      Compounded, freq);
    ...
    yield = BondFunctions::yield(bond1, marketPrice1, bondDayCount, Compounded,
                                 freq);
    ...
    yield = BondFunctions::yield(bond1, marketPrice1, bondDayCount, Continuous,
                                 freq);
    ...
    yield = BondFunctions::yield(bond1, bond1.cleanPrice(), bondDayCount,
    ...
    price = BondFunctions::cleanPrice(bond2, marketYield2, bondDayCount,
                                      Compounded, freq);
    ...
    yield = BondFunctions::yield(bond2, marketPrice2, bondDayCount, Compounded,
                                 freq);
    ...
    yield = BondFunctions::yield(bond2, marketPrice2, bondDayCount, Continuous,
                                 freq);
    ...
    yield = BondFunctions::yield(bond2, bond2.cleanPrice(), bondDayCount,
    ...
    Schedule sch3(Date(30, November, 2004), Date(30, November, 2006),
                  Period(freq), UnitedStates(UnitedStates::GovernmentBond),
                  Unadjusted, Unadjusted, DateGeneration::Backward, false);

    FixedRateBond bond3(settlementDays, vars.faceAmount, sch3,
                        std::vector<Rate>(1, 0.02875),
                        ActualActual(ActualActual::ISMA), ModifiedFollowing,
    ...
    price = BondFunctions::cleanPrice(bond3, marketYield3, bondDayCount,
...
void BondTest::testCachedZero() {
    ...
    ZeroCouponBond bond1(settlementDays,
                         ...
                         vars.faceAmount, Date(30, November, 2008),
    ...
    ZeroCouponBond bond2(settlementDays,
                         UnitedStates(UnitedStates::GovernmentBond),
                         vars.faceAmount, Date(30, November, 2007),
                         ModifiedFollowing, 100.0, Date(30, November, 2004));
    ...
    ZeroCouponBond bond3(settlementDays,
                         UnitedStates(UnitedStates::GovernmentBond),
                         vars.faceAmount, Date(30, November, 2006),
                         ModifiedFollowing, 100.0, Date(30, November, 2004));
...
void BondTest::testCachedFixed() {
    ...
    Schedule sch(Date(30, November, 2004), Date(30, November, 2008),
    ...
    Schedule sch3(
        Date(30, November, 2004), Date(30, March, 2009), Period(Semiannual),
...
void BondTest::testCachedFloating() {
    ...
    Handle<YieldTermStructure> riskFreeRate(
        flatRate(today, 0.025, Actual360()));
    Handle<YieldTermStructure> discountCurve(
        flatRate(today, 0.03, Actual360()));
    ...
    FloatingRateBond bond1(
        settlementDays, vars.faceAmount, sch, index,
    ...
    FloatingRateBond bond3(settlementDays, vars.faceAmount, sch, index,
                           ActualActual(ActualActual::ISMA), ModifiedFollowing,
                           fixingDays, std::vector<Real>(), spreads,
                           std::vector<Rate>(), std::vector<Rate>(), false,
                           100.0, Date(30, November, 2004));
...
void BondTest::testBrazilianCached() {
    ...
    for (Size bondIndex = 0; bondIndex < maturityDates.size(); bondIndex++) {
        ...
        InterestRate yield(yields[bondIndex], Business252(Brazil()), Compounded,
                           Annual);

        Schedule schedule(Date(1, January, 2007), maturityDates[bondIndex],
                          Period(Semiannual), Brazil(Brazil::Settlement),
                          Unadjusted, Unadjusted, DateGeneration::Backward,
                          false);

        FixedRateBond bond(settlementDays, faceAmount, schedule, couponRates,
                           Following, redemption, issueDate);

        Real cachedPrice = prices[bondIndex];
        Real price = faceAmount *
                     (BondFunctions::cleanPrice(
                          bond, yield.rate(), yield.dayCounter(),
...
void BondTest::testExCouponGilt() {
    ...
    */
    ...
    FixedRateBond bond(settlementDays, 100.0,
                       Schedule(startDate, maturityDate, tenor, NullCalendar(),
                                Unadjusted, Unadjusted, DateGeneration::Forward,
                                true, firstCouponDate),
                       std::vector<Rate>(1, coupon), dc, Unadjusted, 100.0,
                       issueDate, calendar, exCouponPeriod, calendar);

    const Leg &leg = bond.cashflows();

    test_case cases[] = {{Date(29, May, 2013), 103.0, 3.8021978, 106.8021978,
                          0.0749518, 5.6760445, 42.1531486},
                         {Date(30, May, 2013), 103.0, -0.1758242, 102.8241758,
                          0.0749618, 5.8928163, 43.7562186},
                         {Date(31, May, 2013), 103.0, -0.1538462, 102.8461538,
                          0.0749599, 5.8901860, 43.7239438}};

    for (Size i = 0; i < LENGTH(cases); ++i) {
        Real accrued = bond.accruedAmount(cases[i].settlementDate);
        ASSERT_CLOSE("accrued amount", cases[i].settlementDate, accrued,
                     cases[i].accruedAmount, 1e-6);

        Real npv = cases[i].testPrice + accrued;
        ASSERT_CLOSE("NPV", cases[i].settlementDate, npv, cases[i].NPV, 1e-6);

        Rate yield = CashFlows::yield(leg, npv, dc, comp, freq, false,
                                      cases[i].settlementDate);
        ASSERT_CLOSE("yield", cases[i].settlementDate, yield, cases[i].yield,
                     1e-6);

        Time duration =
            CashFlows::duration(leg, yield, dc, comp, freq, Duration::Modified,
                                false, cases[i].settlementDate);
        ASSERT_CLOSE("duration", cases[i].settlementDate, duration,
                     cases[i].duration, 1e-6);

        Real convexity = CashFlows::convexity(leg, yield, dc, comp, freq, false,
                                              cases[i].settlementDate);
        ASSERT_CLOSE("convexity", cases[i].settlementDate, convexity,
                     cases[i].convexity, 1e-6);

        Real calcnpv = CashFlows::npv(leg, yield, dc, comp, freq, false,
                                      cases[i].settlementDate);
        ASSERT_CLOSE("NPV from yield", cases[i].settlementDate, calcnpv,
                     cases[i].NPV, 1e-6);

        Real calcprice = calcnpv - accrued;
        ASSERT_CLOSE("price from yield", cases[i].settlementDate, calcprice,
                     cases[i].testPrice, 1e-6);
    }
}
...
void BondTest::testExCouponAustralianBond() {
    ...
    */
    ...
    test_case cases[] = {{Date(7, August, 2014), 103.0, 2.8670, 105.867,
                          0.04723, 2.26276, 6.54870},
                         {Date(8, August, 2014), 103.0, -0.1160, 102.884,
                          0.047235, 2.32536, 6.72531},
                         {Date(11, August, 2014), 103.0, -0.0660, 102.934,
                          0.04719, 2.31732, 6.68407}};

    for (Size i = 0; i < LENGTH(cases); ++i) {
        Real accrued = bond.accruedAmount(cases[i].settlementDate);
        ASSERT_CLOSE("accrued amount", cases[i].settlementDate, accrued,
                     cases[i].accruedAmount, 1e-3);
        ...
        Time duration =
            CashFlows::duration(leg, yield, dc, comp, freq, Duration::Modified,
                                false, cases[i].settlementDate);
        ASSERT_CLOSE("duration", cases[i].settlementDate, duration,
                     cases[i].duration, 1e-5);

        Real convexity = CashFlows::convexity(leg, yield, dc, comp, freq, false,
                                              cases[i].settlementDate);
        ASSERT_CLOSE("convexity", cases[i].settlementDate, convexity,
                     cases[i].convexity, 1e-4);
