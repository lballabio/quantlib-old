...
\subsection{\label{sec:TOY}The Turn of Year Effect}
\par
Smooth interpolation allows for a correct treatment of the so called turn of year effect.
This is a jump normally observed in market quotations for interest rate instruments with an underlying rate tenor that crosses the end of the year.
Such effect is due essentially to the increased search for liquidity by financial institutions at the beginning of the year for balance sheet reasons.
\par
The larger jump is observed in the Overnight Deposit that crosses the end of the year. This enters in all deposits with larger maturity that crosses the same border. We may think to the 1M deposit, for instance, as an average of 22 (business days in one month) 1D rates plus a basis, in which one single rate crosses the end of the year and displays the jump, while the others do not. So, the overnight jump weight is 1/22. For rates with longer tenors the jumping overnight weights less. Hence longer deposits display smaller jumps.
\par
An yield curve including the turn of year effect must display discontinuities, essentially at the end of the current and the next year.
Such effect can be modeled through a multiplicative coefficient applied to all discount factors corresponding to dates following a given end of year.
In this way we are allowed to estimate the coefficient for a given curve, and to apply it to any curve.
...
\par
Gli approcci empirici utilizzati per la stima degli scavalchi di fine d'anno sono molteplici ed utilizzati contemporaneamente, sebbene non tutti siano utilizzabili in tutti i periodi dell'anno a causa delle peculiarità degli strumenti quotati che non variano con la distanza dalla prossima fine d'anno. Tutti gli approcci, quando utilizzabili, danno luogo a stime in eccellente accordo tra loro. Gli approcci sono:
"	Salto nella strip del 3M Euribor come ricavabile dai futures corretti per il convexity adjustment. Si considera quale sarebbe il livello del forward che scavalca l'anno ottenuto interpolando tra i forward che non scavalcano l'anno. La differenza tra questo valore e quello quotato sul mercato è l'effto di fine d'anno. Questo metodo consente di ottenere sempre il secondo scavalco di fine d'anno, ed il primo scavalco fino a settembre. Dopo settembre per il primo bisognerebbe estrapolare e l'approccio non è robusto.
"	Salto nella strip dei FRA 6M. Il metodo è equivalente al precedente sul 3M e consente la stima per primo scavalco di fine d'anno fino a giugno
"	Salto nella strip dei FRA 1M ottenuta dagli swap mesali. Il metodo è equivalente a quello illustrato per il 3M ed il 6M e consente la stima per primo scavalco di fine d'anno fino a novembre
"	Rilevazione del salto nella strip di FRA CROSS quotata dai broker ogni lunedì. Questo metodo vale tutto l'anno, ma da un'aggiornamento solo settimanale e non continuo come i metodi precedenti.
