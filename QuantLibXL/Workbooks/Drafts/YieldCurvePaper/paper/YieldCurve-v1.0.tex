...
&\simeq& \frac{P_x\left(t,T_0\right) - P_x\left(t,T_n\right)}{A_x\left(t,\mathbf{T}\right)}
...
Actually the telescopic property holds exactly with regular dates grid $\mathbf{S}$, such that periods corresponding to consecutive forward rates do concatenate exactly, with no gaps or overlaps. In practice, dates $S_j$ are adjusted with their relevant business day convention, and the resulting periods do not match exactly. Typically, such date mismatch does not exceed one business day (which sometimes can be three calendar days). Hence the second line in eq. \ref{eqn:SwapRateFwd} is just an approximation of the exact expression in the first line.
