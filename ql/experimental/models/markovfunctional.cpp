...
namespace QuantLib {
	...
	const Real MarkovFunctional::deflatedZerobond(Time T, Time t, Real y, boost::shared_ptr<Interpolation> discountSpread) const {

		calculate();
		Array ya(1,y);
		return deflatedZerobond(T,t,ya,discountSpread )[0];

	}

	const Disposable<Array> MarkovFunctional::deflatedZerobond(const Time T, const Time t, const Array& y, boost::shared_ptr<Interpolation> discountSpread ) const {
