...
namespace QuantLib {
   ...
   GsrProcess::GsrProcess(const Array& times, const Array& vols, const Array& reversions, const Real T)
    : times_(times), vols_(vols), reversions_(reversions), revZero_(reversions.size(),false), ForwardMeasureProcess1D(T) {
		...
		for(int i=0;i<(int)reversions.size();i++) if(close(reversions[i],0.0)) revZero_[i]=true;
		flushCache();
    }
	...
	Real GsrProcess::drift(Time t, Real x) const { 
		QL_REQUIRE(t<=getForwardMeasureTime(),"t (" << t << ") must not be greater than forward measure time (" << getForwardMeasureTime() << ")");
		return y(t)-G(t,this->getForwardMeasureTime(),x)*vol(lowerIndex(t))*vol(lowerIndex(t))-rev(lowerIndex(t))*x;
	}
	...
	Real GsrProcess::expectationp1(Time w, Real xw, Time dt) const {
		...
		std::map<std::tuple<Real,Real>,Real>::const_iterator k = cache1_.find(key);
		if(k!=cache1_.end()) return xw*(k->second);
		// A(w,t)x(w)
		Real res2=1.0;
	...
	Real GsrProcess::expectationp2(Time w, Time dt) const {
		...
		std::tuple<Real,Real> key;
		key = std::make_tuple(w,t);
		std::map<std::tuple<Real,Real>,Real>::const_iterator k = cache2_.find(key);
		if(k!=cache2_.end()) return k->second;
		...
		for(int k=lowerIndex(w);k<=upperIndex(t)-1;k++) {
			...
			for(int l=0;l<=k-1;l++) {
				Real res2=1.0;
				//alpha_l
				res2 *= revZero_[l] ? vol(l)*vol(l)*(time(l+1)-time(l)) : vol(l)*vol(l)/(2.0*rev(l))*(1.0-exp(-2.0*rev(l)*(time(l+1)-time(l))));
				//zeta_i (i>k)
				...
				//zeta_k beta_k
				res2 *= revZero_[k] ? 2.0*time(k)-flooredTime(k,w)-cappedTime(k+1,t) - 2.0*(time(k)-cappedTime(k+1,t)) : 
			...
			Real res2=1.0;
			// alpha_k zeta_k
			res2 *= revZero_[k] ? vol(k)*vol(k)/4.0*(4.0*pow(cappedTime(k+1,t)-time(k),2.0)-(pow(flooredTime(k,w)-2.0*time(k)+cappedTime(k+1,t),2.0)+
		...
		for(int k=lowerIndex(w);k<=upperIndex(t)-1;k++) {
			...
			for(int l=k+1;l<=upperIndex(T)-1;l++) {
				Real res3=1.0;
				//eta_l
				res3 *= revZero_[l] ? cappedTime(l+1,T)-time(l) : (1.0-exp(-rev(l)*(cappedTime(l+1,T)-time(l))))/rev(l);
				//zeta_i (i>k)
				for(int i=k+1;i<=upperIndex(t)-1;i++)
				...
				//zeta_k gamma_k
				res3 *= revZero_[k] ?  (cappedTime(k+1,t)-time(k+1) - (2.0*flooredTime(k,w)-cappedTime(k+1,t)-time(k+1))) / 2.0 :
			...
			Real res3=1.0;
			// eta_k zeta_k
			res3 *=   revZero_[k] ? (-pow(cappedTime(k+1,t)-cappedTime(k+1,T),2.0) - 2.0*pow(cappedTime(k+1,t)-flooredTime(k,w),2.0) +
				                            pow(2.0*flooredTime(k,w)-cappedTime(k+1,T)-cappedTime(k+1,t),2.0)) / 4.0 :
				      ( 2.0 - exp(rev(k)*(cappedTime(k+1,t)-cappedTime(k+1,T))) - 
	...
	Real GsrProcess::variance(Time w, Real, Time dt) const { 
		...
		std::tuple<Real,Real> key;
		key = std::make_tuple(w,t);
		std::map<std::tuple<Real,Real>,Real>::const_iterator k = cache3_.find(key);
		if(k!=cache3_.end()) return k->second;
		...
		for(int k=lowerIndex(w);k<=upperIndex(t)-1;k++) {
			...
			// zeta_k^2
			res2 *= revZero_[k] ? -(flooredTime(k,w)-cappedTime(k+1,t)) : ( 1.0 - exp( 2.0*rev(k)*(flooredTime(k,w)-cappedTime(k+1,t))) ) / (2.0*rev(k));
			// zeta_i (i>k)
			for(int i=k+1;i<=upperIndex(t)-1;i++) {
	...
	Real GsrProcess:: y(Time t) const {
		...
		QL_REQUIRE(t>=0.0 && t<=getForwardMeasureTime(), "y(t) should be called with t (" << t << ") in Range [0," << getForwardMeasureTime() << "].");

		Real key;
		key = t;
		std::map<Real,Real>::const_iterator k = cache4_.find(key);
		if(k!=cache4_.end()) return k->second;
		...
		for(int i=0; i<=upperIndex(t)-1; i++) {
			...
			res2 *= revZero_[i] ? vol(i)*vol(i) * (cappedTime(i+1,t)-time(i)) : (vol(i)*vol(i)/(2.0*rev(i))*(1.0-exp(-2.0*rev(i)*(cappedTime(i+1,t)-time(i)))));
			res +=res2;
		}
	...
	Real GsrProcess::G(Time t, Time w, Real) const {
		...
		QL_REQUIRE(t>=0.0 && w<=getForwardMeasureTime(),"G(t,w) should be called with (t,w)=(" << t << "," << w << ") in Range [0," << getForwardMeasureTime() << "].");

		std::tuple<Real,Real> key;
		key = std::make_tuple(w,t);
		std::map<std::tuple<Real,Real>,Real>::const_iterator k = cache5_.find(key);
		if(k!=cache5_.end()) return k->second;
		...
		for(int i=lowerIndex(t); i<=upperIndex(w)-1; i++) {
			...
			res2 *= revZero_[i] ? cappedTime(i+1,w)-flooredTime(i,t) : (1.0-exp(-rev(i)*(cappedTime(i+1,w)-flooredTime(i,t))))/rev(i);
			res +=res2;
		}
	...
	const int GsrProcess::lowerIndex(Time t) const {
	   return std::upper_bound(times_.begin(), times_.end(), t) - times_.begin();
	}

	const int GsrProcess::upperIndex(Time t) const {
	   if(t<QL_EPSILON) return 0;
	   return std::upper_bound(times_.begin(), times_.end(), t - QL_EPSILON) - times_.begin() +1;
	}
	...
	const Real GsrProcess::vol(Size index) const {
		if( index >= vols_.size() ) return vols_.back();
		return vols_[index];
	}

	const Real GsrProcess::rev(Size index) const {
		if( index >= reversions_.size() ) return reversions_.back();
